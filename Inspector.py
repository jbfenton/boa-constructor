...
from Preferences import IS, inspPageNames, keyDefs, oiLineHeight
...
[
    wxID_INSPECTORFRAME,
    wxID_INSPECTORFRAMECONSTR,
    wxID_INSPECTORFRAMEEVENTS,
    wxID_INSPECTORFRAMEPAGES,
    wxID_INSPECTORFRAMEPROPS,
    wxID_INSPECTORFRAMESTATUSBAR,
    wxID_INSPECTORFRAMETOOLBAR,
...
class InspectorFrame(wx.Frame, Utils.FrameRestorerMixin):
    ...
    _custom_classes = {
        "wx.Notebook": ["InspectorNotebook"],
        "wx.SplitterWindow": ["EventsWindow"],
        "wx.ScrolledWindow": ["InspectorPropScrollWin", "InspectorConstrScrollWin"],
    }

    def _init_coll_pages_Pages(self, parent):
        # generated method, don't edit

        parent.AddPage(imageId=-1, page=self.constr, select=True, text=self.constr_name)
        parent.AddPage(imageId=-1, page=self.props, select=False, text=self.props_name)
        parent.AddPage(imageId=-1, page=self.events, select=False, text=self.events_name)
    ...
    def _init_coll_toolBar_Tools(self, parent):
        ...
        parent.AddTool(
            toolId=wxID_INSPECTORFRAMETOOLBARTOOLS0,
            label="",
            bitmap=self.up_bmp,
    ...
    def _init_ctrls(self, prnt):
        ...
        wx.Frame.__init__(
            self,
            id=wxID_INSPECTORFRAME,
            name="",
            parent=prnt,
            pos=wx.Point(363, 272),
            size=wx.Size(290, 505),
        ...
        self.statusBar = wx.StatusBar(
            id=wxID_INSPECTORFRAMESTATUSBAR, name="statusBar", parent=self, style=wx.STB_SIZEGRIP
        )
        self.statusBar.SetFont(
            wx.Font(
                Preferences.inspStatBarFontSize,
        ...
        self.pages = InspectorNotebook(
            id=wxID_INSPECTORFRAMEPAGES, name="pages", parent=self, pos=wx.Point(0, 24), size=wx.Size(282, 434), style=0
        )

        self.constr = InspectorConstrScrollWin(
            id=wxID_INSPECTORFRAMECONSTR,
            name="constr",
            parent=self.pages,
            pos=wx.Point(0, 0),
            size=wx.Size(274, 408),
            style=wx.SUNKEN_BORDER,
        )

        self.props = InspectorPropScrollWin(
            id=wxID_INSPECTORFRAMEPROPS,
            name="props",
            parent=self.pages,
            pos=wx.Point(0, 0),
            size=wx.Size(274, 408),
        ...
        self.events = EventsWindow(
            id=wxID_INSPECTORFRAMEEVENTS,
            name="events",
            parent=self.pages,
            pos=wx.Point(0, 0),
            size=wx.Size(274, 404),
            style=wx.SP_3D,
        )
    ...
    def __init__(self, parent):
        ...
        Utils.AddToolButtonBmpIS(self, self.toolBar, "Images/Shared/Delete.png", _("Delete selection"), self.OnDelete)
        Utils.AddToolButtonBmpIS(self, self.toolBar, "Images/Shared/Cut.png", _("Cut selection"), self.OnCut)
        Utils.AddToolButtonBmpIS(self, self.toolBar, "Images/Shared/Copy.png", _("Copy selection"), self.OnCopy)
        Utils.AddToolButtonBmpIS(self, self.toolBar, "Images/Shared/Paste.png", _("Paste selection"), self.OnPaste)
        Utils.AddToolButtonBmpIS(
            self, self.toolBar, "Images/Editor/Refresh.png", _("Recreate selection"), self.OnRecreateSelection
        )
        self.toolBar.AddSeparator()
        self.wxID_POST = Utils.AddToolButtonBmpIS(
            self, self.toolBar, "Images/Inspector/Post.png", _("Post the session"), self.OnPost
        )
        self.wxID_CANCEL = Utils.AddToolButtonBmpIS(
            self, self.toolBar, "Images/Inspector/Cancel.png", _("Cancel the session"), self.OnCancel
        )
        self.toolBar.AddSeparator()
        ##        Utils.AddToolButtonBmpIS(self, self.toolBar, 'Images/Shared/RevertItem.png',
        ##          'Revert item', self.OnRevertItem)
        self.wxID_ADDITEM = Utils.AddToolButtonBmpIS(
            self, self.toolBar, "Images/Shared/NewItem.png", _("New item"), self.OnNewItem
        )
        self.wxID_DELITEM = Utils.AddToolButtonBmpIS(
            self, self.toolBar, "Images/Shared/DeleteItem.png", _("Delete item"), self.OnDelItem
        )
        self.toolBar.AddSeparator()
        Utils.AddToolButtonBmpIS(self, self.toolBar, "Images/Shared/Help.png", _("Show help"), self.OnHelp)
    ...
    def setDefaultDimensions(self):
        self.SetSize(
            Preferences.screenX,
            Preferences.underPalette + Preferences.screenY,
    ...
    # ---Object selection------------------------------------------------------------
    def selectObject(
        self, compn, selectInContainment=True, collDesgn=None, focusPage=None, restore=False, sessionHandler=None
    ):
        ...
        if (
            ...
            and compn.designer
            and not collDesgn
            and (compn.designer != self.prevDesigner[0] or not collDesgn and self.prevDesigner[1])
        ):
    ...
    def updateToolBarState(self):
        canAddDel = (
            self.selCmp is not None and hasattr(self.selCmp, "propItems") and hasattr(self.selCmp, "updateZopeProps")
        )
    ...
    def OnRevertItem(self, event):
        if self.selCmp and self.props.prevSel and self.props.prevSel.propEditor:
            ...
            propEdit.companion.propRevertToDefault(propEdit.name, propEdit.propWrapper.getSetterName())
        ...
        if self.selCmp and hasattr(self.selCmp, "propItems"):
            ...
            try:
                if dlg.ShowModal() == wx.ID_OK:
                    self.selCmp.addProperty(
                        dlg.tcPropName.GetValue(), dlg.tcValue.GetValue(), dlg.chType.GetStringSelection()
                    )
...
class ParentTree(wx.TreeCtrl):
    ...
    def __init__(self, parent):
        wx.TreeCtrl.__init__(
            self, parent, wxID_PARENTTREE, style=wx.TR_HAS_BUTTONS | wx.CLIP_CHILDREN | wx.SUNKEN_BORDER
        )
        self.cleanup()
    ...
    def refreshCtrl(self, root, relDict, designer):
        ...
        self.root = self.AddRoot(root, PaletteStore.compInfo[designer.objects[""][1].__class__.__bases__[0]][2])
...
class NameValue:
    ...
    def __init__(
        ...
        lockEditor, attrName, self.isCat = self.checkLockedProperty(name, propWrapper.getSetterName(), companion)
        ...
        if editor:
            ...
            self.propEditor = self.inspector.inspector.propertyRegistry.factory(
                name, valueParent, companion, rootCompanion, propWrapper, idx, valueParent.GetSize().x + IECWidthFudge
            )
        ...
        if self.propEditor:
            ...
            if PropertyEditors.esExpandable in self.propEditor.getStyle():
                mID = wx.NewIdRef()
                self.expander = wx.CheckBox(
                    nameParent, mID, "", wx.Point(8 * self.indent, self.idx * oiLineHeight + 2), wx.Size(13, 14)
                )
        ...
        self.nameCtrl = wx.lib.stattext.GenStaticText(
            nameParent,
            -1,
            name,
        ...
        self.value = wx.lib.stattext.GenStaticText(
            valueParent,
            -1,
            displayVal,
            wx.Point(2, idx * oiLineHeight + 2),
            wx.Size(inspector.getValueWidth(), oiLineHeight - 3),
        ...
        self.separatorN = wx.Window(
            nameParent, -1, wx.Point(0, (idx + 1) * oiLineHeight), wx.Size(inspector.panelNames.GetSize().x, 1)
        )  # ,
        # style=wx.CLIP_CHILDREN)
        self.separatorN.SetBackgroundColour(sepCol)
        self.separatorN.Refresh()

        self.separatorV = wx.Window(
            valueParent, -1, wx.Point(0, (idx + 1) * oiLineHeight), wx.Size(inspector.getValueWidth(), 1)
        )  # ,
        # style=wx.CLIP_CHILDREN)
            ...
            if srcVal is not None:
                ...
                if isinstance(srcVal, type([])):
                    if len(srcVal):
                        ...
                        srcVal = ""
                if srcVal.startswith("self.") and hasattr(companion.designer.model.specialAttrs["self"], srcVal[5:]):
    ...
    def setPos(self, idx):
        self.idx = idx
        if self.expander:
            self.expander.SetPosition(wx.Point(8 * self.indent, self.idx * oiLineHeight + 2))
    ...
    def showEdit(self):
        ...
        self.nameBevelBottom = wx.Window(
            self.nameParent,
            -1,
        ...
        if not self.locked and self.propEditor:
            ...
            self.valueBevelBottom = wx.Window(
                self.valueParent,
                -1,
                wx.Point(0, (self.idx + 1) * oiLineHeight - 1),
                wx.Size(self.inspector.getValueWidth(), 1),
        ...
        if self.valueBevelTop:
            self.valueBevelTop.Refresh()
        if self.valueBevelBottom:
            self.valueBevelBottom.Refresh()
            ...
            if cancel:
                ...
                if noUpdate:
                    ...
                    try:
                        ...
                        wx.LogError(
                            _("Could not post %s because: %s: %s") % (self.propName, err.__class__.__name__, str(err))
                        )
                else:
                    self.propEditor.inspectorPost()
                    self.updateDisplayValue()
                    self.value.SetSize(wx.Size(self.separatorV.GetSize().x, oiLineHeight - 3))
...
class EventsWindow(wx.SplitterWindow):
    ...
    def __init__(self, *_args, **_kwargs):
        wx.SplitterWindow.__init__(self, _kwargs["parent"], _kwargs["id"], style=Preferences.splitterStyle)

        self.categories = wx.SplitterWindow(self, -1, style=wx.SP_3D | wx.SP_LIVE_UPDATE)
        self.definitions = InspectorEventScrollWin(self, -1, style=wx.SUNKEN_BORDER | wx.TAB_TRAVERSAL)
        ...
        self.selCatClass = -1
        self.categoryClasses.Bind(wx.EVT_LIST_ITEM_SELECTED, self.OnCatClassSelect, id=wxID_EVTCATS)
        self.categoryClasses.Bind(wx.EVT_LIST_ITEM_DESELECTED, self.OnCatClassDeselect, id=wxID_EVTCATS)

        self.categoryMacros = wx.ListCtrl(self.categories, wxID_EVTMACS, style=wx.LC_LIST)
        self.categoryMacros.Bind(wx.EVT_LIST_ITEM_SELECTED, self.OnMacClassSelect, id=wxID_EVTMACS)
        self.categoryMacros.Bind(wx.EVT_LIST_ITEM_DESELECTED, self.OnMacClassDeselect, id=wxID_EVTMACS)
    ...
    def findMacro(self, name):
        for macro in EventCollections.EventCategories[self.categoryClasses.GetItemText(self.selCatClass)]:
            if macro == name:
                return macro
    ...
    def OnCatClassSelect(self, event):
        ...
        catClass = EventCollections.EventCategories[self.categoryClasses.GetItemText(self.selCatClass)]
...
class NameValueEditorScrollWin(wx.ScrolledWindow):
    ...
    def __init__(
        ...
        wx.ScrolledWindow.__init__(self, parent, id, style=style)  # | wxTAB_TRAVERSAL)
        self.nameValues = []
        self.prevSel = None
        self.splitter = wx.SplitterWindow(
            self, -1, wx.Point(0, 0), parent.GetSize(), style=Preferences.splitterStyle | wx.SP_3D
        )

        self.panelNames = wx.Panel(self.splitter, -1, wx.DefaultPosition, wx.Size(100, 1), style=0)  # wxTAB_TRAVERSAL)
    ...
    def deleteNameValues(self, idx, count, cancel=False):
        ...
        if idx < len(self.nameValues):
            ...
            while (idx < len(self.nameValues)) and (deleted < count):
                if self.nameValues[idx] == self.prevSel:
                    self.prevSel = None
    ...
    def collapse(self, nameValue):
        ...
        if self.nameValues[0].propValue in ["wx.ListView", " wx.ListCtrl"]:
            ...
            if selectedModeStyleCount != 1:
                ...
                if selectedModeStyleCount == 0:
                    ...
                    print(
                        "There is a problem with your style flags. You have not selected a mode for the control.\n"
                        ...
                        + "Please select a mode for this control. "
                        + "(You may have to consult wxPython documentation to determine which one you want)"
                    )
                else:
                    # too many modes selected. advise user accordingly.
                    print(
                        "There is a problem with your style flags. You have selected too many modes for the control\n"
                        + "There can only be EXACTLY ONE mode setting. This can be any of;\n\n"
                        + "wx.LC_LIST, wx.LC_REPORT, wx.LC_VIRTUAL, wx.LC_ICON and wx.LC_SMALL_ICON.\n\n"
                        + "Please select a single mode for this control. "
                        + "(You may have to consult wxPython documentation to determine which one you want)"
                    )
        ...
        if doCollapse:
            ...
            if i < len(self.nameValues):
                while (i < len(self.nameValues)) and (self.nameValues[i].indent > startIndent):
                    i = i + 1
            count = i - idx
...
class InspectorPropScrollWin(InspectorScrollWin):
    ...
    def readObject(self, propList):
        self.setNameValues(self.inspector.selCmp, self.inspector.selCmp, propList, 0, 0)

    def expand(self, nameValue):
        nv = self.nameValues[nameValue.idx]
        obj = nv.propValue
        compn = nv.propEditor.getSubCompanion()(
            nameValue.propName, self.inspector.selCmp.designer, self.inspector.selCmp, obj, nv.propEditor.propWrapper
        )
...
class InspectorConstrScrollWin(InspectorScrollWin):
    ...
    def expand(self, nameValue):
        ...
        for propWrap in propLst:
            if propWrap:
                self.addProp(propWrap.name, compn, self.inspector.selCmp, propWrap, indt, startIdx, nv.propEditor)
                startIdx = startIdx + 1
        nv.propEditor.expanded = True
        nv.updateDisplayValue()
        self.refreshSplitter()
        ...
        if nv:
            ...
            self.nameValues.insert(
                ...
                EventNameValue(
                    ...
                    self.inspector.selCmp,
                    name,
                    RTTI.PropertyWrapper(
                        name, "EventRoute", self.inspector.selCmp.evtGetter, self.inspector.selCmp.evtSetter
                    ),
                    len(self.nameValues),
                    -2,
                    PropertyEditors.EventPropEdit,
                ),
            )
...
class InspectorNotebook(wx.Notebook):
    ...
    def __init__(self, *_args, **_kwargs):
        wx.Notebook.__init__(self, _kwargs["parent"], _kwargs["id"], style=Preferences.inspNotebookFlags)
        self.pages = {}
