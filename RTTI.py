...
class PropertyWrapper:
    ...
    def __repr__(self):
        return "<instance PropertyWrapper: %s, %s (%s, %s)" % (self.name, self.routeType, self.getter, self.setter)
...
def traverseAndBuildProps(props, vetoes, obj, Class):
    for m in Class.__dict__.keys():
        if m not in vetoes:
            cat, name, methGetter, methSetter = getMethodType(m, obj, Class.__dict__)
