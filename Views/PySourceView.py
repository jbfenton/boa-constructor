...
from .StyledTextCtrls import (
    AutoCompleteCodeHelpSTCMix,
    BrowseStyledTextCtrlMix,
    CallTipCodeHelpSTCMix,
    DebuggingViewSTCMix,
    FoldingStyledTextCtrlMix,
    PythonStyledTextCtrlMix,
    idWord,
    object_delim,
    word_delim,
)
...
class ShellNameNotFound(Exception):
    pass


class PythonSourceView(
    EditorStyledTextCtrl,
    PythonStyledTextCtrlMix,
    BrowseStyledTextCtrlMix,
    FoldingStyledTextCtrlMix,
    AutoCompleteCodeHelpSTCMix,
    ...
    def __init__(self, parent, model):
        ...
        EditorStyledTextCtrl.__init__(self, parent, wxID_PYTHONSOURCEVIEW, model, a1, -1)
        PythonStyledTextCtrlMix.__init__(self, wxID_PYTHONSOURCEVIEW, (lineNoMrg, Preferences.STCLineNumMarginWidth))
        ...
        DebuggingViewSTCMix.__init__(self, (brkPtMrk, tmpBrkPtMrk, disabledBrkPtMrk, stepPosMrk))
    ...
    def processDedent(self, textLst, idntBlock):
        ...
        for idx in range(len(textLst)):
            if len(textLst[idx]) >= indentLevel and textLst[idx][:indentLevel] == idntBlock:
    ...
    def getTipValue(self, word, lnNo):
        ...
        if (len(objPth) == 2 or len(objPth) == 3) and objPth[0] == "wx":
            wxPyTip = self.getFirstContinousBlock(self.checkWxPyTips(module, word))
            if wxPyTip:
                return wxPyTip
        ...
        if cls:
            if len(objPth) == 2 and objPth[0] == "self":
                if objPth[1] in cls.methods:
                    return self.prepareModSigTip(objPth[1], cls.methods[objPth[1]].signature)
                elif cls.super and isinstance(cls.super[0], type("")):
                    return self.getFirstContinousBlock(self.checkWxPyMethodTips(module, cls.super[0], objPth[1]))
            ...
            if len(objPth) == 3 and objPth[0] == "self":
                return self.getFirstContinousBlock(self.getAttribSig(module, cls, objPth[1], objPth[2]))
    ...
    def getNameSig(self, name, module):
        if name in module.classes and "__init__" in module.classes[name].methods:
            return self.prepareModSigTip(name, module.classes[name].methods["__init__"].signature)
    ...
    def getAttribSig(self, module, cls, attrib, meth):
        if attrib in cls.attributes:
            ...
            if objtype in module.classes and meth in module.classes[objtype].methods:
    ...
    sigTypeMap = {"dict": dict, "list": list, "string": str, "tuple": tuple, "number": int}
    ...
    def getTypeSig(self, objType, method, module):
        ...
        if objType in module.classes and method in module.classes[objType].methods:
            return self.prepareModSigTip(method, module.classes[objType].methods[method].signature)
    ...
    def prepareWxModSigTip(self, tip):
        ...
        if paramStart != -1:
            paramEnd = tip.rfind(")")
            if paramEnd != -1:
                return self.prepareModSigTip(tip[:paramStart], tip[paramStart + 1 : paramEnd]).strip()
        return tip.strip()
    ...
    def getImportedShellObj(self, words):
        module = self.model.getModule()
        imports = (
            list(module.imports.keys()) + list(module.from_imports.keys()) + list(module.from_imports_names.keys())
        )
        if self.model.editor.shell:
    ...
    attrTypeMap = {"dict": dir({}), "list": dir([]), "string": dir(""), "tuple": dir(()), "number": dir(0)}
    ...
    def handleBrwsWxNames(self, module, word, words, currLineNo):
        ...
        if wxObj is not None:
            if hasattr(wxObj, "__module__"):
                try:
                    path, impType = self.model.findModule(wxObj.__module__)
                except ImportError:
                    return False, None
                else:
                    if impType == "package":
                        ...
                        if impType == "name":
                            model.views["Source"].gotoName(model.getModule(), words[-1], currLineNo)
                        return True, model
        return False, None
    ...
    def handleBrwsImports(self, module, word, currLineNo):
        ...
        if word in module.imports:
            ...
            try:
                path, impType = self.model.findModule(word)
            except ImportError:
                return False, None
            else:
                if impType == "package":
        ...
        # Handle name part of from module import name
            ...
            try:
                path, impType = self.model.findModule(modName, word)
            except ImportError:
                return False, None
            else:
                self.doClearBrwsLn()
                ...
                model, ctrlr = self.model.editor.openOrGotoModule(path)
                if impType == "name":
                    model.views["Source"].gotoName(model.getModule(), word, currLineNo)
                return True, model
        else:
            ...
            if len(words) > 1:
                ...
                if handled is None:
                    ...
                    if not model.views["Source"].gotoName(model.getModule(), words[-1], currLineNo):
    ...
    def handleBrwsCheckImportStars(self, module, word, currLineNo):
        ...
        if not module.from_imports_star_cache:
            for starMod in module.from_imports_star:
                try:
                    module.from_imports_star_cache[starMod] = self.model.findModule(starMod)
                except ImportError:
                    pass
    ...
    def checkChangesAndSyntax(self, lineNo=None):
        ...
        if not Preferences.onlyCheckIfLineModified or lineNo == self.damagedLine:
            slb, sle = (
                self.GetStyleAt(self.PositionFromLine(lineNo)),
                self.GetStyleAt(self.GetLineEndPosition(lineNo) - 1),
            )
            line = self.GetLine(lineNo)
            self.checkSyntax((line,), lineNo + 1, self.GetLine, lineStartStyle=slb, lineEndStyle=sle)
    ...
    def checkSyntax(
        ...
        if lineStartStyle in list(self.ignore_styles.keys()) or lineEndStyle in list(self.ignore_styles.keys()):
            self.model.editor.statusBar.setHint(errstr)
            return
        ...
        compflags = 0
        try:
            compflags = compflags | __future__.generators.compiler_flag
        except AttributeError:
            pass
        ...
        try:
            ...
            if err.msg == "unexpected EOF while parsing":
                ...
                self.checkSyntax(prevlines, lineNo, getPrevLine, "def func():\n", " ")
                return
            ...
            elif err.msg in ("'break' outside loop", "'continue' not properly in loop"):
                self.checkSyntax(prevlines, lineNo, getPrevLine, "while 1:\n", " ")
                return
            elif err.msg == "invalid token":
                self.indicateError(
                    lineNo, err.offset + indentpl - len(indent) - contLinesOffset, "SyntaxError: %s" % err.msg
                )
            ...
            # Invalid syntax
                ...
                if len(prevlines) == 1 and err.offset is not None and not contLinesOffset:
                    ...
                    if possblkeyword in list(self.if_keywords.keys()):
                        self.checkSyntax(
                            (prevline.replace(possblkeyword, self.if_keywords[possblkeyword], 1),), lineNo, getPrevLine
                        )
                        return
                        ...
                        self.checkSyntax((self.try_keywords[possblkeyword], prevline), lineNo, getPrevLine)
                        return

                    # Check for line continueations
                    # XXX Lines ending on line_conts should be ignored
                    errpos = err.offset - len(indent) - 1
                    if errpos < len(stripprevline) and stripprevline[errpos] in self.line_conts_ends:
                        ln = lineNo - 2
                                ...
                                if len(lstripline) >= 4:
                                    ...
                                    if possblifkeyword in list(self.if_keywords.keys()):
                                        ...
                                        rstripline = rstripline.replace(
                                            possblifkeyword, self.if_keywords[possblifkeyword], 1
                                        )
                        ...
                        if errOffsetOffset:
                            self.checkSyntax((lines,), lineNo, getPrevLine, contLinesOffset=errOffsetOffset)
                            return
                ...
                self.indicateError(lineNo, erroffset + indentpl - len(indent) - contLinesOffset, errstr)
                self.damagedLine = -1
                return
    ...
    def OnRunToCursor(self, event):
        ...
        if self.model.app:
            ...
            model = self.model
        model.debug(cont_if_running=1, cont_always=1, temp_breakpoint=temp_breakpoint)
    ...
    def OnCodeTransform(self, event):
        ...
        if word.startswith("self."):
            ...
            if cls and methName not in cls.methods:
                ...
                if len(methName) > 2 and methName[:2] == "On" and (methName[2] in string.uppercase + "_"):
                    parms = "self, event"
                else:
                    parms = "self, "
            ...
            if line[:startOffset].strip() == "def":
                ...
                if cls and cls.super:
                    ...
                    if meth:
                        # XXX Not tab aware
                        module.addLine("%s%s.%s(%s)" % (" " * startOffset, baseName, word, meth.signature), lnNo + 1)
                        self.model.refreshFromModule()
                        self.model.modified = True
                        self.model.editor.updateModulePage(self.model)
...
class PythonDisView(EditorStyledTextCtrl, PythonStyledTextCtrlMix):
    ...
    def __init__(self, parent, model):
        ...
        EditorStyledTextCtrl.__init__(self, parent, wxID_PYTHONDISVIEW, model, (), -1)
...
ExplorerNodes.langStyleInfoReg.insert(0, ("Python", "python", PythonStyledTextCtrlMix, "stc-styles.rc.cfg"))
