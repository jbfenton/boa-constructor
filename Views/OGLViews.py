...
class PersistentShapeCanvas(ogl.ShapeCanvas):
    ...
    def loadSizes(self, filename):
        ...
        try:
            ...
            if (
                wx.MessageBox(
                    _("%s is possibly corrupt (cannot be unpickled), delete it?Default layout will be used.")
                    % filename,
                    _("Corrupt file"),
...
class ScrollingContainer(wx.ScrolledWindow):
    ...
    def __init__(self, parent, size):
        ...
        self.SetScrollbars(self.scrollStepX, self.scrollStepY, size.x / self.scrollStepX, size.y / self.scrollStepY)
...
class PersistentOGLView(ScrollingContainer, EditorViews.EditorView):
    ...
    def OnSetSize(self, event):
        dlg = wx.TextEntryDialog(self, "Enter new canvas size (width==height)", "Size", repr(self.size))
        try:
...
class SortedUMLViewMix:
    ...
    def buildShapes(self, dc):
        ...
        for parent, child in routes:
            self.newLine(
                dc,
                self.getCurrentShape(child),
                self.getCurrentShape(parent),
            )
...
class UMLView(PersistentOGLView, SortedUMLViewMix):
    ...
    def __init__(self, parent, model):
        ...
        id = wx.NewIdRef(count=1)
        self.menuStdClass.Append(
            id,
            _("Goto Documentation"),
        )
    ...
    def newClass(self, size, pos, className, classMeths, classAttrs):
        ...
        maxWidth = 10  # padding
        if not self.showAttributes:
            classAttrs = [" "]
        if not self.showMethods:
            classMeths = [" "]

        regionName, maxWidth, nameHeight = self.newRegion(Preferences.oglBoldFont, "class_name", [className], maxWidth)
        regionAttribs, maxWidth, attribsHeight = self.newRegion(
            Preferences.oglStdFont, "attributes", classAttrs, maxWidth
        )
        regionMeths, maxWidth, methsHeight = self.newRegion(Preferences.oglStdFont, "methods", classMeths, maxWidth)
        ...
        idx = self.addShape(shape, pos[0], pos[1], Preferences.vpOGLClassShapePen, Preferences.vpOGLClassShapeBrush, "")
    ...
    def newExternalClass(self, size, pos, className):
        ...
        idx = self.addShape(
            shape, pos[0], pos[1], Preferences.vpOGLExternalClassShapePen, Preferences.vpOGLExternalClassShapeBrush, ""
        )
    ...
    def OnToggleAttributes(self, event):
        if self.showAttributes == 1:
            self.showAttributes = 0
        else:
            self.showAttributes = 1
        return self.refreshCtrl()

    def OnGotoDoc(self, event):
        if self.menuClass:
            name = self.menuClass
            if "Documentation" in self.model.views and name in self.model.getModule().classes:
                srcView = self.model.views["Documentation"]
                srcView.focus()
                self.model.getModule()
...
class ImportsView(PersistentOGLView):
    ...
    def __init__(self, parent, model):
        PersistentOGLView.__init__(
            self, parent, model, (("-", None, "", ""), (_("Refresh"), self.OnRefresh, self.refreshBmp, ""))
        )
    ...
    def newModule(self, size, pos, moduleName, importList):
        idx = self.addShape(
            PerstDividedShape(moduleName, size[0], size[1]),
            pos[0],
            pos[1],
        ...
        regionClss, maxWidth, clssHeight = self.newRegion(Preferences.oglStdFont, "methods", importList, maxWidth)
...
class AppPackageView(PersistentOGLView):
    ...
    def newModule(self, size, pos, moduleName, importList):
        idx = self.addShape(
            PerstDividedShape(moduleName, size[0], size[1]), pos[0], pos[1], wx.BLACK_PEN, wx.LIGHT_GREY_BRUSH, ""
        )
