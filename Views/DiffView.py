...
from .EditorViews import CloseableViewMix, EditorView
...
class PythonSourceDiffView(wx.stc.StyledTextCtrl, EditorView, PythonStyledTextCtrlMix, CloseableViewMix):
    viewName = "Diff"
    viewTitle = _("Diff")
    ...
    def __init__(self, parent, model):
        ...
        wx.stc.StyledTextCtrl.__init__(
            self, parent, wxID_PYTHONSOURCEDIFFVIEW, style=wx.CLIP_CHILDREN | wx.SUNKEN_BORDER
        )
        ...
        EditorView.__init__(
            ...
            + (
                ...
                (_("Apply all changes"), self.OnApplyAllChanges, "-", ""),
            ),
            -1,
        )
    ...
    def OnMarginClick(self, event):
        if event.GetMargin() == 1:
            self.LineFromPosition(event.GetPosition())

    def OnPrev(self, event):
        self.currSearchLine = self.MarkerPrevious(self.currSearchLine, maskMarkSet) - 1
        self.gotoLine(self.currSearchLine + 1)

    def OnNext(self, event):
        self.currSearchLine = self.MarkerNext(self.currSearchLine, maskMarkSet) + 1
        self.gotoLine(self.currSearchLine - 1)
    ...
    def OnApplyAllChanges(self, event):
        ...
        if self.diffWith:
            ...
            if Utils.yesNoDialog(self, _("Are you sure?"), _("Replace %s with %s?") % (filename, diffWith)):
