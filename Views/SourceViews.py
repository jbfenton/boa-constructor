...
from .StyledTextCtrls import TextSTCMix
...
class EditorStyledTextCtrl(wx.stc.StyledTextCtrl, EditorViews.EditorView, EditorViews.FindResultsAdderMixin):
    ...
    def __init__(self, parent, wId, model, actions, defaultAction=-1):
        ...
        a = (
            ...
            ("-", None, "", ""),
            (_(r"Find \ Replace"), self.OnFind, self.findBmp, "Find"),
    ...
    def refreshCtrl(self):
        ...
        if not self.eolsChecked:
            if Utils.checkMixedEOLs(str(newData)):
                wx.LogWarning(
                    _("Mixed EOLs detected in %s, please use Edit->Convert... to fix this problem.")
    ...
    def gotoLine(self, lineno, offset=-1):
        ...
        if offset != -1:
            self.SetCurrentPos(self.GetCurrentPos() + offset + 1)
    ...
    def processSelectionBlock(self, func):
        ...
        try:
            ...
            self.SetSelection(self.PositionFromLine(sls), self.GetLineEndPosition(sle))
        finally:
            self.EndUndoAction()

    def getSelectionAsLineNumbers(self):
        selStartPos, selEndPos = self.GetSelection()
        self.LineFromPosition(selStartPos)
        self.LineFromPosition(selEndPos)

        return list(range(self.LineFromPosition(selStartPos), self.LineFromPosition(selEndPos)))
    ...
    def doFind(self, pattern):
        self.lastSearchResults = Search.findInText(self.GetText().split(self.eol), pattern, False)
        self.lastSearchPattern = pattern
        if len(self.lastSearchResults):
            self.lastMatchPosition = 0

    def doNextMatch(self):
        if self.lastMatchPosition is not None and len(self.lastSearchResults) > self.lastMatchPosition:
    ...
    def OnMarkPlace(self, event):
        ...
        try:
            ...
            wx.Yield()
            time.sleep(0.125)
            # self.MarkerDelete(lineno, markPlaceMrk)   # Seems to remove marker immediately after adding it.
            #                                             for now.
        finally:
            self._marking = False
    ...
    def OnChangeEOLMode(self, event):
        eol = {
            wxID_STC_EOL_CRLF: wx.stc.STC_EOL_CRLF,
            wxID_STC_EOL_LF: wx.stc.STC_EOL_LF,
            wxID_STC_EOL_CR: wx.stc.STC_EOL_CR,
        }[event.GetId()]
    ...
    def OnConvertEols(self, event):
        eol, viewEOL = {
            wxID_CVT_EOL_CRLF: (wx.stc.STC_EOL_CRLF, "\r\n"),
            wxID_CVT_EOL_LF: (wx.stc.STC_EOL_LF, "\n"),
            wxID_CVT_EOL_CR: (wx.stc.STC_EOL_CR, "\r"),
        }[event.GetId()]
...
class TextView(EditorStyledTextCtrl, TextSTCMix):
    ...
    def __init__(self, parent, model, actions=()):
        EditorStyledTextCtrl.__init__(self, parent, wxID_TEXTVIEW, model, actions, -1)
