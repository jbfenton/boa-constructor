...
class BevelIEC(InspectorEditorControl):
    ...
    def createControl(self, parent, idx, sizeX):
        self.bevelTop = wx.Panel(parent, -1, (0, idx * Preferences.oiLineHeight - 1), (sizeX, 1))
        self.bevelTop.SetBackgroundColour(wx.BLACK)
        self.bevelBottom = wx.Panel(parent, -1, (0, (idx + 1) * Preferences.oiLineHeight - 1), (sizeX, 1))
...
class BeveledLabelIEC(BevelIEC):
    def createControl(self, parent, idx, sizeX):
        ...
        self.editorCtrl = wx.StaticText(
            parent, -1, self.value, (2, idx * Preferences.oiLineHeight + 2), (sizeX, Preferences.oiLineHeight - 3)
        )
...
class ButtonIEC(BevelIEC):
    ...
    def createControl(self, parent, idx, sizeX, editMeth):
        ...
        self.propValLabel = wx.StaticText(
            parent,
            -1,
            str(self.getValue()),
    ...
    def setWidth(self, width):
        if self.editorCtrl:
            ...
            self.editorCtrl.SetSize(
                width - self.btnSize - 3, self.editorCtrl.GetPosition().y, self.btnSize, Preferences.oiLineHeight - 2
            )
            # self.propValLabel.SetDimensions(2,
            self.propValLabel.SetSize(
                2, self.propValLabel.GetPosition().y, width - self.btnSize - 6, Preferences.oiLineHeight - 3
            )
    ...
    def setIdx(self, idx):
        if self.editorCtrl:
            self.editorCtrl.SetSize(
                self.editorCtrl.GetPosition().x,
                idx * Preferences.oiLineHeight + 2,
                self.btnSize,
                Preferences.oiLineHeight - 2,
            )
            self.propValLabel.SetSize(
                self.propValLabel.GetPosition().x,
                idx * Preferences.oiLineHeight + 1,
                self.propValLabel.GetSize().x,
...
class TextCtrlButtonIEC(BevelIEC):
    def createControl(self, parent, idx, sizeX, editMeth):
        ...
        self.editorCtrl = [
            wx.TextCtrl(
                ...
                value,
                (-2, idx * Preferences.oiLineHeight - 2),
                (sizeX - 20, Preferences.oiLineHeight + 3),
            ),  # , style = style),
            wx.BitmapButton(
                parent,
                self.wID2,
    ...
    def setWidth(self, width):
        if self.editorCtrl:
            self.editorCtrl[0].SetSize(wx.Size(width - 20, self.editorCtrl[0].GetSize().y))
            self.editorCtrl[1].SetDimensions(
                width - 18 - 3, self.editorCtrl[1].GetPosition().y, 18, Preferences.oiLineHeight
            )
...
class CheckBoxIEC2(InspectorEditorControl):
    def createControl(self, parent, idx, sizeX):
        self.editorCtrl = wx.Window(parent, wx.NewIdRef(), style=wx.TAB_TRAVERSAL | wx.SUNKEN_BORDER)
        self.editorCtrl.SetDimensions(-2, idx * Preferences.oiLineHeight - 2, sizeX, Preferences.oiLineHeight + 3)
...
class CheckBoxIEC(BevelIEC):
    def createControl(self, parent, idx, sizeX):
        self.editorCtrl = wx.CheckBox(
            parent, self.wID, "False", (2, idx * Preferences.oiLineHeight + 1), (sizeX, Preferences.oiLineHeight - 2)
        )
        self.editorCtrl.Bind(wx.EVT_CHECKBOX, self.OnSelect, id=self.wID)
    ...
    def setValue(self, value):
        if self.editorCtrl:
            self.editorCtrl.SetLabel(value)
            self.editorCtrl.SetValue(self.TrueFalseMap[True].lower() == value.lower())

    def setIdx(self, idx):
        if self.editorCtrl:
            self.editorCtrl.SetDimensions(
                2, idx * Preferences.oiLineHeight + 1, self.editorCtrl.GetSize().x, Preferences.oiLineHeight - 2
            )
        BevelIEC.setIdx(self, idx)
