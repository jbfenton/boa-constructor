...
class ModuleRunner:
    ...
    def checkError(self, err, caption, out=None, root="Error", errRaw=()):
        if self.esf:
            ...
            self.results = {"err": err, "caption": caption, "out": out, "root": root, "errRaw": errRaw}
...
class ProcessModuleRunner(ModuleRunner):
    ...
    def run(self, cmd, Parser=ErrorStack.StdErrErrorParser, caption=_("Execute module"), root="Error", autoClose=False):
