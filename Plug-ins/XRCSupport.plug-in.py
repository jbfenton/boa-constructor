...
class XrcPanelDTC(ContainerCompanions.PanelDTC):
    ...
    def designTimeDefaults(self, position=wx.DefaultPosition, size=wx.DefaultSize):
        ...
        return {"parent": self.parent, "id": -1, "pos": (posx, posy), "size": (200, 200)}
    ...
    def persistConstr(self, className, params):
        ...
        self.textConstr = methodparse.ConstructorParse(
            "self.%s = self.%s.LoadPanel(%s)" % (self.name, self.xmlResource, ", ".join(paramStrs))
        )
...
Plugins.registerComponent("Utilities (Data)", wx.xrc.XmlResource, "wx.xrc.XmlResource", XmlResourceDTC)
Plugins.registerComponent("ContainersLayout", XrcPanel, "XrcPanel", XrcPanelDTC)
