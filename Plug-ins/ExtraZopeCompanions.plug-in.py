...
import PaletteStore
import Plugins
from Models import Controllers, EditorHelper, HTMLSupport, XMLSupport
from Views import PySourceView, SourceViews
from ZopeLib import ZopeEditorModels
from ZopeLib.ZopeCompanions import DBAdapterZC, ZopeCompanion
...
class TransparentFolderZC(ZopeCompanion):
    def create(self):
        mime, res = self.call(
            self.objPath, "manage_addProduct/TransparentFolder/manage_addTransparentFolder", id=self.name, title=""
        )
...
class LocalFSZC(ZopeCompanion):
    def create(self):
        mime, res = self.call(
            self.objPath, "manage_addProduct/LocalFS/manage_addLocalFS", id=self.name, title="", basepath=""
        )
...
class LFNode(LFSNode):
    ...
    additionalViews = ()
    supportedViews = {
        "Module": (PySourceView.PythonSourceView,),
    ...
    def __init__(self, name, resourcepath, clipboard, imgIdx, parent, xmlrpcsvr, root, properties, metatype):
        Mod = Controllers.identifyFile(os.path.basename(resourcepath), localfs=False)[0]
        imgIdx = Mod.imgIdx
        self.defaultViews = self.supportedViews.get(Mod.modelIdentifier, (SourceViews.TextView,))

        LFSNode.__init__(self, name, resourcepath, clipboard, imgIdx, parent, xmlrpcsvr, root, properties, metatype)
    ...
    def load(self, mode="rb"):
        ...
        r = Resource(("http://%(host)s:%(httpport)s/" + self.resourcepath) % props, props["username"], props["passwd"])
        return r.get().body
    ...
    def save(self, filename, data, mode="wb"):
        ...
        zc.callkw(dirname, "manage_upload", {"file": file, "Content-Type": "multipart/form-data"})
...
class LFDirNode(LFSNode):
    def __init__(self, name, resourcepath, clipboard, imgIdx, parent, xmlrpcsvr, root, properties, metatype):
        LFSNode.__init__(
            self, name, resourcepath, clipboard, EditorHelper.imgFolder, parent, xmlrpcsvr, root, properties, metatype
        )


zopeClassMap.update({"Local File System": LFSNode, "LocalFS::directory": LFDirNode, "LocalFS::file": LFNode})


class ZODBCDAZC(DBAdapterZC):
    def create(self):
        dlg = wx.TextEntryDialog(None, "Enter the Connection String", "Z ODBC DB Adapter", "")
        try:
            if dlg.ShowModal() == wx.ID_OK:
                connId = dlg.GetValue()
                mime, res = self.call(
                    self.objPath, "manage_addZODBCConnection", id=self.name, title="", connection=connId, check=1
                )
        finally:
            dlg.Destroy()
...
PaletteStore.paletteLists["Zope"].extend(["Python Method", "LocalFS", "Transparent Folder", "ZODBCDA"])
