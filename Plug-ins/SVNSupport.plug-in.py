...
def isSVN(filename):
    ...
    return (
        ...
        and os.path.exists(os.path.join(filename, "format"))
    )
...
class SVNController(ExplorerNodes.Controller):
    ...
    def svnCmdPrompt(self, wholeCommand, inDir, help=""):
        ...
        dlg = wx.TextEntryDialog(self.list, "(in dir %s)\n\n%s" % (inDir, help), "SVN command line", wholeCommand)
    ...
    def getSvnHelp(self, cmd, option="help"):
        SVNPD = ProcessProgressDlg.ProcessProgressDlg(self.list, "svn %s %s" % (option, cmd), "", modally=False)
        try:
    ...
    def doSvnCmd(self, cmd, svnDir, stdinput="", svnOutput="output window"):
        ...
        try:
            ...
            if svnOutput == "output window":
                ...
                if err.strip():
                    dlg = wx.MessageDialog(self.list, err, "Server response or Error", wx.OK | wx.ICON_EXCLAMATION)
                    try:
                        dlg.ShowModal()
                    finally:
                        dlg.Destroy()
    ...
    def doSvnCmdOnSelection(self, cmd, cmdOpts, preCmdFunc=None, postCmdFunc=None, svnOutput="output window"):
        if self.list.node:
            ...
            cmdStr = self.svnCmdPrompt(self.svnCmd(cmd, cmdOpts, names), svnDir, self.getSvnHelp(cmd))
            if cmdStr:
                if preCmdFunc:
                    preCmdFunc(names)
    ...
    def doSvnCmdInDir(self, cmd, cmdOpts, svnDir, items, svnOpts=""):
        cmdStr = self.svnCmdPrompt(self.svnCmd(cmd, cmdOpts, items, svnOpts), svnDir, self.getSvnHelp(cmd))
        if cmdStr:
    ...
    def checkoutSVNItems(self):
        ...
        svnDir = self.list.node.resourcepath
        if self.doSvnCmdInDir(
            "checkout", "", svnDir, ["--username [USER]", "--password [PASS]", "[URL]", "[PATH]"], ""
        ):
            self.list.refreshCurrent()
...
class SVNFileNode(ExplorerNodes.ExplorerNode):
    ...
    def __init__(self, entry, resourcepath, parent):
        ...
        ##                self.missing = True
        ##
        self.imgIdx = self.missing and self.missing << 2 or self.conflict * 5 or self.modified << 1
            ...
            if timestamp.startswith("Result of merge+"):
                ...
                if conflicts:
                    ...
                    if SVNConflictsView.viewName not in model.views:
                        resultView = editor.addNewView(SVNConflictsView.viewName, SVNConflictsView)
                    else:
...
class FSSVNFolderNode(ExplorerNodes.ExplorerNode):
    ...
    def createParentNode(self):
        if self.parent:
            ...
            return FSSVNFolderNode(
                os.path.basename(parent), parent, self.clipboard, EditorModels.SVNFolderModel.imgIdx, self
            )
    ...
    def openList(self):
        ...
        for entry in fileEntries:
            if entry.name not in names:
                unaddedNode = SVNUnAddedItem(entry.name, entry.resourcepath, self, entry.isFolderish())
        ...
        ##            else:
        ##                node = res.get(
        ##                    entry.name, SVNUnAddedItem(entry.name, entry.resourcepath, self, entry.isFolderish())
        ##                )
        ##            if node:
        ##                lst.append(node)
        ##
...
FileExplorer.FileSysNode.subExplorerReg["folder"].append((FSSVNFolderNode, isSVN, EditorHelper.imgCVSFolder))

ExplorerNodes.register(FSSVNFolderNode, clipboard=None, confdef=("", ""), controller=SVNController)
