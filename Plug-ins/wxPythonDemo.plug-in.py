...
Plugins.registerPreference(
    "wxPythonDemo",
    "wpShowWxPythonDemoTemplate",
    "False",
    ["Should a template for an empty wxPython demo file", "be available on the Palette."],
)
    ...
    def updateDemoCtrl(self):
        ...
        if demoModule:
            ...
            if self.demoCtrl:
                ...
                self.demoCtrl.SetConstraints(LayoutAnchors(self.demoCtrl, True, True, True, True))
...
class wxPythonDemoNode(FileExplorer.FileSysNode):
    def open(self, editor):
        ...
        if model.demoModule and (model, cntrl) != (None, None):
            for View, focus in ((wxPythonDemoOverView, False), (wxPythonDemoView, True)):
...
class wxPythonDemoSectionNode(ExplorerNodes.ExplorerNode):
    ...
    def __init__(self, name, items, clipboard, bookmarks):
        ExplorerNodes.ExplorerNode.__init__(self, name, "", clipboard, EditorHelper.imgFolder)
    ...
    def createChildNode(self, name):
        ...
        if mod is None:
            ...
            return wxPythonDemoNode(name, demoFile, self.clipboard, imgIdx, self, self.bookmarks)
...
class wxPythonDemoDirNode(ExplorerNodes.ExplorerNode):
    ...
    def __init__(self, clipboard, parent, bookmarks):
        ExplorerNodes.ExplorerNode.__init__(self, _("wxPython demo"), "", clipboard, EditorHelper.imgWxPythonDemo)
        self.bookmarks = bookmarks
        self.bold = true
        self.vetoSort = true
    ...
    def createChildNode(self, section, items):
        return wxPythonDemoSectionNode(section, items, self.clipboard, self.bookmarks)

    def openList(self):
        return [self.createChildNode(name, items) for name, items in self.treeList]
...
from Models import PythonControllers, PythonEditorModels
