...
class CodeTemplateManagerDlg(wx.Dialog):
    def _init_ctrls(self, prnt):
        ...
        wx.Dialog.__init__(
            self,
            id=wxID_CODETEMPLATEMANAGERDLG,
            name="CodeTemplateManagerDlg",
            parent=prnt,
            pos=wx.Point(639, 363),
            size=wx.Size(349, 324),
...
class CodeTemplateManager:
    ...
    def readTemplates(self):
        ...
        for path in Preferences.pluginPaths:
            ...
            if os.path.exists(cp):
                files.extend([os.path.join(cp, f) for f in glob.glob(os.path.join(cp, "*.cfg"))])
    ...
    def getNames(self, language):
        ...
        names = [name for name in names if self.conf.get(name, "language") in (language, "any")]
        names.sort()
        return names
...
class CodeTemplatesViewPlugin:
    ...
    def OnCodeTemplateSelected(self, event):
        if event.GetListType() == codeTemplatesUserListType:
            ...
            self.view.insertCodeBlock(codeTemplateManager.getTemplate(event.GetText()).replace("\\t", idnt))
        event.Skip()
