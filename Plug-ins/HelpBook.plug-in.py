...
def createIndexDlg(parent, keyword, location, anchors):
    return HelpBookItemDlg(
        parent, _("Help Book - Index"), keyword, _("Keyword(s)"), "", location, _("Location"), anchors
    )


def createContentsDlg(parent, title, htmlTitle, location, anchors):
    return HelpBookItemDlg(
        parent, _("Help Book - Contents"), title, _("Title"), htmlTitle, location, _("Location"), anchors
    )
...
class HelpBookItemDlg(wx.Dialog):
    def _init_ctrls(self, prnt):
        ...
        self.staticText1 = wx.StaticText(
            id=wxID_HELPBOOKITEMDLGSTATICTEXT1,
        ...
        self.txtTitle = wx.TextCtrl(
            id=wxID_HELPBOOKITEMDLGTXTTITLE,
            name="txtTitle",
            parent=self,
            pos=wx.Point(72, 8),
            size=wx.Size(232, 21),
        ...
        self.staticText2 = wx.StaticText(
            id=wxID_HELPBOOKITEMDLGSTATICTEXT2,
        ...
        self.txtPage = wx.TextCtrl(
            id=wxID_HELPBOOKITEMDLGTXTPAGE,
            name="txtPage",
            parent=self,
            pos=wx.Point(72, 40),
            size=wx.Size(232, 21),
        ...
        self.cbbAnchors = wx.ComboBox(
            choices=self.valueAnchors,
    ...
    def __init__(self, parent, caption, name, nameLabel, nameDefault, value, valueLabel, valueAnchors):
...
class HelpConfigParser:
    ...
    def generateConfigData(self):
        ...
        res.append(
            'wxHelp=,"%(Contents file)s","%(Index file)s",'
    ...
    def updateFilenameOptions(self, name):
        self.options.update(
            {"Contents file": name + ".hhc", "Compiled file": name + ".chm", "Index file": name + ".hhk"}
        )
...
class HelpBookModel(EditorModels.SourceModel):
    ...
    def load(self, notify=True):
        try:
            self.contentsModel.load(False)
        except ExplorerNodes.TransportError:
            pass

        try:
            self.indexesModel.load(False)
        except ExplorerNodes.TransportError:
            pass
...
class HelpBookFilesView(EditorViews.VirtualListCtrlView):
    ...
    def __init__(self, parent, model, provideActions=True):
        if provideActions:
            ...
            actions = ()
        EditorViews.VirtualListCtrlView.__init__(self, parent, model, wx.LC_REPORT, actions, -1)
    ...
    helpBookDirMsg = _(
        "Help documents must be in the same directory as the help "
    ...
    def OnOpenFile(self, event):
        ...
        if idx != -1:
            ...
            self.model.editor.openOrGotoModule(helpBookDir + "/" + self.model.config.files[idx])
    ...
    def OnNormalisePaths(self, event):
        ...
        delidxs = []
        for idx, filename in zip(list(range(len(self.model.config.files))), self.model.config.files):
            absfn = os.path.normpath(os.path.join(helpBookDir, filename))
            if absfn[: len(helpBookDir)] != helpBookDir:
                if (
                    wx.MessageBox(
                        _('Filename "%s" not inside help book directory.\n\nRemove?') % absfn,
                        _("Invalid file"),
...
class HelpBookIndexView(wx.SplitterWindow, EditorViews.EditorView):
    ...
    def __init__(self, parent, model):
        wx.SplitterWindow.__init__(self, parent, -1, style=wx.CLIP_CHILDREN | wx.SP_3DSASH)
...
class HelpBookIndexListView(EditorViews.VirtualListCtrlView):
    def __init__(self, parent, model, parentView=None):
        EditorViews.VirtualListCtrlView.__init__(self, parent, model, wx.LC_REPORT, (), -1)
    ...
    def OnEditIndex(self, event):
        if self.selected != -1:
            ...
            docsDir = os.path.dirname(self.model.filename)
            try:
                data = Explorer.openEx(os.path.join(docsDir, location.split("#")[0])).load()
...
class HelpBookContentsView(wx.SplitterWindow, EditorViews.EditorView):
    ...
    def __init__(self, parent, model):
        ...
        actions = (
            (_("Edit entry"), self.contents.OnEditEntry, "-", ""),
...
class HelpBookContentsTreeView(wx.TreeCtrl, EditorViews.EditorView):
    ...
    def __init__(self, parent, model, parentView=None):
        wx.TreeCtrl.__init__(self, parent, -1, style=wx.TR_HAS_BUTTONS | wx.SUNKEN_BORDER | wx.TR_DEFAULT_STYLE)
    ...
    def refreshCtrl(self):
        ...
        rootItem = self.AddRoot(title, 0, data=(title, None, self.model.contents))
    ...
    def recurseAddItems(self, items, treeItem):
        ...
        for name, value, children in items:
            item = self.AppendItem(treeItem, name, children and 1 or 2, -1, wx.TreeItemData((value, items, children)))
            if children:
    ...
    def OnDeleteEntry(self, event):
        ...
        if wx.MessageBox(_("Delete %s") % name, _("Delete node and children"), wx.ICON_WARNING | wx.YES_NO) == wx.YES:
            del items[idx]
...
class HelpBookController(Controllers.SourceController):
    ...
    def OnMakeHTB(self, event):
        ...
        dlg = wx.SingleChoiceDialog(
            model.editor, _("Choose source files"), _("Make HTB"), [_("Files list"), _("Entire help book directory")]
        )
        try:
            if dlg.ShowModal() != wx.ID_OK:
                return
        ...
        if selected == 0:
            ...
            files = []
            os.path.walk(docsDir, visitDir, (files, [os.path.basename(zipfilename)]))
