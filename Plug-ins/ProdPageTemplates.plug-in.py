...
from Models.HTMLSupport import HTMLFileController, HTMLFileModel, HTMLFileView
from ZopeLib.ZopeEditorModels import ZOAIcons, ZopeController, ZopeDocumentModel, addZOAImage
...
class ZopePTHTMLView(ZopeViews.ZopeHTMLView):
    ...
    def generatePage(self):
        ...
        url = "http://%s:%s@%s:%d/%s/source.html" % (
            props["username"],
            props["passwd"],
            props["host"],
            props["httpport"],
...
class PageTemplateNode(ZopeNode):
    ...
    additionalViews = (ZopeViews.ZopeUndoView, ZopeViews.ZopeSecurityView, ZopePTHTMLView, ZopeViews.ZopeHTMLView)
...
class PageTemplateZC(CustomZopePropsMixIn, ZopeCompanion):
    def create(self):
        ...
        mime, res = self.call(self.objPath, prodPath + "manage_addPageTemplate", id=self.name, title="")
