...
class ListCtrlDTC(Constructors.MultiItemCtrlsConstr, WindowDTC):
    def __init__(self, name, designer, parent, ctrlClass):
        ...
        self.editors.update(
            {
                "Columns": ListColumnsColPropEdit,
        ...
        self.listTypeNameMap = {"ImageListSmall": wx.IMAGE_LIST_SMALL, "ImageListNormal": wx.IMAGE_LIST_NORMAL}
    ...
    def designTimeSource(self, position="wx.DefaultPosition", size="wx.DefaultSize"):
        return {"pos": position, "size": self.getDefCtrlSize(), "style": "wx.LC_ICON", "name": repr(self.name)}
    ...
    def GetImageList(self, name):
        return (self.control.GetImageList(self.listTypeNameMap[name]), self.listTypeNameMap[name])
            ...
            for propName, typeName in (
                ...
                if imgLst and repr(imgLst) == repr(compn.control):
                    ...
                    while idx < len(self.textPropList):
                        ...
                        if prop.prop_setter == "SetImageList" and prop.params[1] == typeName:
    ...
    def persistProp(self, name, setterName, value):
        if setterName == "SetImageList":
            ...
            self.textPropList.append(
                methodparse.PropertyParse(None, self.name, setterName, [imgList, listType], "SetImageList")
            )
        else:
...
class ListCtrlColumnsCDTC(CollectionDTC):
    ...
    def constructor(self):
        return {"Column": "col", "Heading": "heading", "Format": "format", "Width": "width"}
        ...
        props.update(
            {
                ...
                "Width": ("IndexRoute", wx.ListCtrl.GetColumnWidth, wx.ListCtrl.SetColumnWidth),
            }
        )
        return props
...
class TreeCtrlDTC(Constructors.MultiItemCtrlsConstr, WindowDTC):
    ...
    def designTimeSource(self, position="wx.DefaultPosition", size="wx.DefaultSize"):
        return {"pos": position, "size": size, "style": "wx.TR_HAS_BUTTONS", "name": repr(self.name)}
...
class ListBoxDTC(Constructors.ListConstr, ChoicedDTC):
    ...
    def designTimeSource(self, position="wx.DefaultPosition", size="wx.DefaultSize"):
        return {"pos": position, "size": size, "choices": "[]", "style": "0", "name": repr(self.name)}
...
class GridDTC(WindowDTC, Constructors.WindowConstr):
    ...
    def properties(self):
        ...
        props.update(
            {
                "Editable": ("CtrlRoute", wx.grid.Grid.IsEditable, wx.grid.Grid.EnableEditing),
                "GridLinesEnabled": ("CtrlRoute", wx.grid.Grid.GridLinesEnabled, wx.grid.Grid.EnableGridLines),
            }
        )
        return props
    ...
    def writeImports(self):
        return "\n".join((WindowDTC.writeImports(self), "import wx.grid"))
