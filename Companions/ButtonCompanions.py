...
class ButtonDTC(WindowDTC):
    ...
    def constructor(self):
        return {"Position": "pos", "Size": "size", "Label": "label", "Style": "style", "Name": "name", "Id": "id"}
        ...
        if name == "Default":
            for prop in self.textPropList:
                if prop.prop_setter == setterName:
                    ...
                    return
            if value.lower() == "true":
                self.textPropList.append(methodparse.PropertyParse(None, self.getCompName(), setterName, [], name))
        else:
...
class BitmapButtonDTC(WindowDTC):
    ...
    def properties(self):
        props = WindowDTC.properties(self)
        props.update({"Bitmap": ("CtrlRoute", wx.BitmapButton.GetBitmapLabel, wx.BitmapButton.SetBitmapLabel)})
        return props
...
class SpinButtonDTC(WindowDTC, Constructors.WindowConstr):
    def __init__(self, name, designer, parent, ctrlClass):
        WindowDTC.__init__(self, name, designer, parent, ctrlClass)
        self.windowStyles = ["wx.SP_HORIZONTAL", "wx.SP_VERTICAL", "wx.SP_ARROW_KEYS", "wx.SP_WRAP"] + self.windowStyles
    ...
    def designTimeSource(self, position="wx.DefaultPosition", size="wx.DefaultSize"):
        return {"pos": position, "size": size, "style": "wx.SP_HORIZONTAL", "name": repr(self.name)}
        ...
        if setterName == "SetRange":
            ...
            for prop in self.textPropList:
                if prop.prop_setter == setterName:
                    ...
                    return
            # add if not defined
            self.textPropList.append(methodparse.PropertyParse(None, self.name, setterName, newParams, "SetRange"))
        else:
            WindowDTC.persistProp(self, name, setterName, value)
...
class GenButtonDTC(WindowDTC):
    ...
    def constructor(self):
        return {"Label": "label", "Position": "pos", "Size": "size", "Style": "style", "Name": "name"}

    def designTimeSource(self, position="wx.DefaultPosition", size="wx.DefaultSize"):
        return {"label": repr(self.name), "pos": position, "size": size, "name": repr(self.name), "style": "0"}

    def events(self):
        return WindowDTC.events(self) + ["ButtonEvent"]

    def writeImports(self):
        return "\n".join((WindowDTC.writeImports(self), "import wx.lib.buttons"))
...
class GenBitmapButtonDTC(GenButtonDTC):
    ...
    def constructor(self):
        return {"BitmapLabel": "bitmap", "Position": "pos", "Size": "size", "Style": "style", "Name": "name"}

    def designTimeSource(self, position="wx.DefaultPosition", size="wx.DefaultSize"):
        return {"bitmap": "wx.NullBitmap", "pos": position, "size": size, "name": repr(self.name), "style": "0"}
...
class ColourPickerCtrlDTC(PickerCtrlDTC):
    def __init__(self, name, designer, parent, ctrlClass):
        ...
        self.windowStyles = ["wx.CLRP_DEFAULT_STYLE", "wx.CLRP_USE_TEXTCTRL", "wx.CLRP_SHOW_LABEL"] + self.windowStyles
    ...
    def constructor(self):
        return {"Position": "pos", "Size": "size", "Style": "style", "Colour": "col", "Name": "name"}
...
class FontPickerCtrlDTC(PickerCtrlDTC):
    ...
    def constructor(self):
        return {"Position": "pos", "Size": "size", "Style": "style", "Font": "initial", "Name": "name"}
