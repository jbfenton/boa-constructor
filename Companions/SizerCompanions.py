...
from PropEdit.PropertyEditors import (
    ...
    IntConstrPropEdit,
    IntPropEdit,
    ObjEnumConstrPropEdit,
    ReadOnlyConstrPropEdit,
    SizeConstrPropEdit,
...
from .BaseCompanions import CollectionDTC, HelperDTC, UtilityDTC


class SizerItemsColPropEdit(CollectionPropEdit):
    pass
...
class SizerDTC(UtilityDTC):
    ...
    def recreateSizer(self):
        args = self.designer.setupArgs(
            self.name, self.textConstr.params, self.handledConstrParams, evalDct=self.designer.model.specialAttrs
        )
...
class SizerItemsCDTC(CollectionDTC):
    ...
    def applyDesignTimeDefaults(self, params, method=None):
        ...
        if (method == "Add" and params[0] == "None") or (method == "Insert" and params[1] == "None"):
            ...
            self.control.Add(BlankSizer(), proportion, defaults["flag"], defaults["border"])
        else:
    ...
    def writeCollectionItems(self, output, stripFrmId=""):
        ...
        for constr in self.textConstrLst:
            if constr.params[0] == "None":
                wx.LogWarning(_("No control/sizer for sizer item of %s") % (self.parentCompanion.name))
                warn = 1
        if warn:
            wx.LogWarning(
                _(
...
class BoxSizerDTC(SizerDTC):
    def __init__(self, name, designer, objClass):
        ...
        self.names["Orientation"] = {"wx.VERTICAL": wx.VERTICAL, "wx.HORIZONTAL": wx.HORIZONTAL}
...
class GridSizerDTC(SizerDTC):
    def constructor(self):
        return {"Name": "name", "Rows": "rows", "Cols": "cols", "VGap": "vgap", "HGap": "hgap"}
...
class GrowablesColPropEdit(CollectionPropEdit):
    def edit(self, event):
        ce = self.companion.designer.showCollectionEditor(self.companion.name, self.name, False)
        ...
        if not numRows or not numCols:
            numItems = len(
                self.companion.designer.showCollectionEditor(
                    self.companion.name, "Items", False
                ).companion.textConstrLst
            )
        else:
            numItems = -1
...
class GrowablesCDTC(CollectionDTC):
    ...
    additionalMethods = {
        "AddGrowableRow": ("Add growable row", 0, "(None)"),
    ...
    def _appendItem(self, method, idx):
        ...
        if method is None:
            method = self.insertionMethod
        collItemInit = methodparse.CollectionItemInitParse(None, self.sourceObjName, method, {0: str(idx)})
...
class FlexGridSizerDTC(GridSizerDTC):
    def __init__(self, name, designer, objClass):
        ...
        self.names["NonFlexibleGrowMode"] = {
        ...
        self.options["NonFlexibleGrowMode"] = [wx.FLEX_GROWMODE_NONE, wx.FLEX_GROWMODE_SPECIFIED, wx.FLEX_GROWMODE_ALL]
...
class ControlLinkedSizerDTC(SizerDTC):
    ...
    def designTimeObject(self, args=None):
        if args is not None:
            ...
            dlg = wx.SingleChoiceDialog(self.designer, _("Choose control"), _("Create sizer"), names)
            try:
    ...
    def notification(self, compn, action):
        if action == "delete" and compn != self:
            compnSrcRef = Utils.srcRefFromCtrlName(compn.name)
            if self.ctrlParam in self.textConstr.params and compnSrcRef == self.textConstr.params[self.ctrlParam]:
        ...
        if args and "box" in args:
            ...
            if b is not None:
                if not b:  # deleted static box
                    args["box"] = wx.StaticBox(
                        self._staticBoxParent, -1, self._staticBoxLabel, name=self._staticBoxName
                    )
                else:
...
class GBSizerItemsCDTC(SizerItemsCDTC):
    def __init__(self, name, designer, parentCompanion, ctrl):
        ...
        self.editors.update({"Position": PositionTuplePropEdit, "Span": SpanTuplePropEdit})

    def constructor(self):
        tcl = self.textConstrLst[self.index]
        if tcl.method == "AddWindow":
            return {"Window": 0, "Position": 1, "Span": "span", "Flag": "flag", "Border": "border"}
        ...
        #             'Span': 'span', 'Flag': 'flag', 'Border': 'border'}
            ...
            return {"Size": 0, "Position": 1, "Span": "span", "Flag": "flag", "Border": "border"}
    ...
    def designTimeSource(self, wId, method=None):
        ...
        if method == "AddWindow":
            return {0: "None", 1: repr(pos), "span": "(1, 1)", "flag": "0", "border": "0"}
        # elif method == 'AddSizer':    # AddSizer not used in wxPython any more.  PRUNE
        #     return {0: 'None', 1: repr(pos), 'span': '(1, 1)',
        #             'flag': '0', 'border': '0'}
        elif method == "AddSpacer":
            return {0: "wx.Size(8, 8)", 1: repr(pos), "span": "(1, 1)", "flag": "0", "border": "0"}
    ...
    def applyDesignTimeDefaults(self, params, method=None):
        ...
        # if (method in ('AddWindow', 'AddSizer') and params[0] == 'None') or \   # orig
        if (method in ("AddWindow", "Add") and params[0] == "None") or (method == "Insert" and params[1] == "None"):
            defaults = self.designTimeDefaults(params, method)
            # self.control.AddSizer(BlankSizer(), defaults[1],    # orig
            self.control.Add(BlankSizer(), defaults[1], defaults["span"], defaults["flag"], defaults["border"])
        else:
...
class RowColDTC(HelperDTC):
    ...
    def __init__(self, name, designer, cmpn, obj, ownerPW):
        ...
        self.editors = {"Row": PositiveIntPropEdit, "Column": PositiveIntPropEdit}

    def properties(self):
        return {
            "Row": ("NameRoute", self.GetRowCol, self.SetRowCol),
