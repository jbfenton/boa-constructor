...
from PropEdit.PropertyEditors import (
    BitmapConstrPropEdit,
    BoolPropEdit,
    CollectionPropEdit,
    ColourConstrPropEdit,
    ConstrPropEdit,
    EnumConstrPropEdit,
    IntConstrPropEdit,
    MenuEnumConstrPropEdit,
    StrConstrPropEdit,
)
from Utils import _

from . import EventCollections
from .BaseCompanions import CollectionDTC, CollectionIddDTC, UtilityDTC
...
class ImageListImagesCDTC(CollectionDTC):
    ...
    additionalMethods = {
        "AddWithColourMask": ("Add with colour mask", "bitmap", "(None)"),
    ...
    def properties(self):
        ...
        props.update(
            {
                "Bitmap": ("CompnRoute", None, self.setBitmap),
    ...
    def designTimeSource(self, wId, method=None):
        ...
        if method == "Add":
            return {"bitmap": "wx.NullBitmap", "mask": "wx.NullBitmap"}
        elif method == "AddWithColourMask":
            return {"bitmap": "wx.NullBitmap", "maskColour": "wx.Colour(255, 0, 255)"}
    ...
    def designTimeDefaults(self, vals, method=None):
        ...
        for param in list(vals.keys()):
            if vals[param] == "wx.NullBitmap" and (dtd[param].GetWidth() != ix or dtd[param].GetHeight() != iy):
...
class AcceleratorTableEntriesCDTC(CollectionDTC):
    ...
    def persistCollInit(self, method, ctrlName, propName, params={}):
        collInitParse = methodparse.CollectionInitParse(None, "[]", method, [], propName)
...
class MenuDTC(UtilityDTC):
    def __init__(self, name, designer, objClass):
        UtilityDTC.__init__(self, name, designer, objClass)
        self.editors.update({"Title": StrConstrPropEdit, "Items": CollectionPropEdit})
    ...
    def hideDesignTime(self):
        return ["NextHandler", "PreviousHandler", "EventHandler", "Id", "Parent", "InvokingWindow"]
...
class MenuItemsCIDTC(CollectionIddDTC):
    ...
    additionalMethods = {
        "AppendSeparator": ("Append separator", "", "(None)"),
    ...
    def constructor(self):
        ...
        if tcl.method == "Append":
            ...
            return {"Text": "item", "Help": "helpString", "Kind": "kind", "ItemId": "id"}
        elif tcl.method == "AppendSeparator":
            return {}
        elif tcl.method == "AppendMenu":
            return {"Text": "text", "Help": "help", "Menu": "submenu", "ItemId": "id"}
    ...
    def designTimeSource(self, wId, method=None):
        ...
        if method == "Append":
            ...
            return {"id": winId, "text": repr(newItemName), "submenu": "wx.Menu()", "help": repr("")}
...
class MenuBarDTC(UtilityDTC):
    ...
    def vetoedMethods(self):
        return UtilityDTC.vetoedMethods(self) + [
            "GetPosition",
            "SetPosition",
            "GetSize",
            "SetSize",
...
class MenuBarMenusCDTC(CollectionDTC):
    ...
    def __init__(self, name, designer, parentCompanion, ctrl):
        CollectionDTC.__init__(self, name, designer, parentCompanion, ctrl)
        self.editors.update({"Menu": MenuEnumConstrPropEdit, "Title": StrConstrPropEdit})

    def constructor(self):
        return {"Menu": "menu", "Title": "title"}

    def designTimeSource(self, wId, method=None):
        return {"menu": "wx.Menu()", "title": repr("%s%d" % (self.propName, wId))}
...
cursorIconTypes = ["wx.BITMAP_TYPE_XBM", "wx.BITMAP_TYPE_CUR", "wx.BITMAP_TYPE_CUR_RESOURCE", "wx.BITMAP_TYPE_ICO"]
...
class CursorDTC(UtilityDTC):
    ...
    def constructor(self):
        return {"CursorName": "cursorName", "Flags": "flags", "HotSpotX": "hotSpotX", "HotSpotY": "hotSpotY"}

    def designTimeSource(self, position="wx.DefaultPosition", size="wx.DefaultSize"):
        return {"cursorName": "''", "flags": "wx.BITMAP_TYPE_CUR", "hotSpotX": "0", "hotSpotY": "0"}
...
class StockCursorDTC(UtilityDTC):
    ...
    def __init__(self, name, designer, objClass):
        UtilityDTC.__init__(self, name, designer, objClass)
        self.editors.update({"CursorId": EnumConstrPropEdit, "Visible": BoolPropEdit})
