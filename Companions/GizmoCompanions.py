...
from . import BaseCompanions, Constructors, ContainerCompanions, EventCollections
...
class DynamicSashWindowDTC(BaseCompanions.WindowDTC, GizmoDTCMix, Constructors.WindowConstr):
    ...
    def designTimeSource(self, position="wx.DefaultPosition", size="wx.DefaultSize"):
        return {"pos": position, "size": "wx.Size(100, 100)", "style": "wx.CLIP_CHILDREN", "name": repr(self.name)}

    def events(self):
        return BaseCompanions.WindowDTC.events(self) + ["DynamicSashEvent"]

    def writeImports(self):
        return "\n".join((BaseCompanions.WindowDTC.writeImports(self), GizmoDTCMix.writeImports(self)))


# -------------------------------------------------------------------------------

LEDNumberCtrlAlignment = [
...
LEDNumberCtrlAlignmentNames = {
    "wx.gizmos.LED_ALIGN_LEFT": wx.gizmos.LED_ALIGN_LEFT,
...
class LEDNumberCtrlDTC(GizmoDTCMix, BaseCompanions.WindowDTC):
    def __init__(self, name, designer, parent, ctrlClass):
        BaseCompanions.WindowDTC.__init__(self, name, designer, parent, ctrlClass)
        self.editors.update({"Alignment": PropertyEditors.EnumPropEdit, "DrawFaded": PropertyEditors.BoolPropEdit})
        self.windowStyles = [
            "wx.gizmos.LED_ALIGN_LEFT",
            "wx.gizmos.LED_ALIGN_RIGHT",
            "wx.gizmos.LED_ALIGN_CENTER",
    ...
    def designTimeSource(self, position="wx.DefaultPosition", size="wx.DefaultSize"):
        return {
            "pos": position,
            "size": "wx.Size(%d, %d)" % (Preferences.dsDefaultControlSize.x, Preferences.dsDefaultControlSize.y),
...
class EditableListBoxDTC(GizmoDTCMix, ContainerCompanions.PanelDTC):
    ...
    def designTimeSource(self, position="wx.DefaultPosition", size="wx.DefaultSize"):
        return {"label": repr(self.name), "pos": position, "size": self.getDefCtrlSize(), "name": repr(self.name)}

    def writeImports(self):
        return "\n".join((ContainerCompanions.PanelDTC.writeImports(self), GizmoDTCMix.writeImports(self)))
...
class TreeListCtrlDTC(GizmoDTCMix, ListCompanions.TreeCtrlDTC):
    def __init__(self, name, designer, parent, ctrlClass):
        ...
        self.editors.update(
            {
                "Columns": TreeListColumnsColPropEdit,
    ...
    def designTimeSource(self, position="wx.DefaultPosition", size="wx.DefaultSize"):
        return ListCompanions.TreeCtrlDTC.designTimeSource(self, position, self.getDefCtrlSize())

    def defaultAction(self):
        self.designer.inspector.props.getNameValue("Columns").propEditor.edit(None)


class TreeListColumnsColPropEdit(PropertyEditors.CollectionPropEdit):
    pass
...
class TreeListCtrlColumnsCDTC(BaseCompanions.CollectionDTC):
    ...
    def properties(self):
        ...
        props.update(
            {"Text": ("IndexRoute", wx.gizmos.TreeListCtrl.GetColumnText, wx.gizmos.TreeListCtrl.SetColumnText)}
        )
        return props
...
Plugins.registerComponent(
    "ContainersLayout", wx.gizmos.DynamicSashWindow, "wx.gizmos.DynamicSashWindow", DynamicSashWindowDTC
)
Plugins.registerComponent("BasicControls", wx.gizmos.LEDNumberCtrl, "wx.gizmos.LEDNumberCtrl", LEDNumberCtrlDTC)
Plugins.registerComponent("ListControls", wx.adv.EditableListBox, "wx.adv.EditableListBox", EditableListBoxDTC)
Plugins.registerComponent("ListControls", wx.gizmos.TreeListCtrl, "wx.gizmos.TreeListCtrl", TreeListCtrlDTC)
