...
from PropEdit import BitmapListEditorDlg, InspectorEditorControls, MaskedEditFmtCodeDlg, PropertyEditors
from Utils import _

from . import BasicCompanions, Constructors, ContainerCompanions, EventCollections
from .BaseCompanions import WindowDTC
from .BasicCompanions import ComboBoxDTC, StaticTextDTC, TextCtrlDTC
...
class AutoFormatPropMixin:
    dependents = ["mask", "datestyle", "formatcodes", "description", "excludeChars", "validRegex"]
    ...
    def __init__(self):
        ...
        self.mutualDepProps += ["Autoformat"] + [s[0].upper() + s[1:] for s in self.dependents]

    def properties(self):
        props = {"Autoformat": ("CompnRoute", self.GetAutoformat, self.SetAutoformat)}
        return props
...
class MaskedDTCMixin:
    def __init__(self):
        ...
        self.editors.update(
            {
                "AutoCompleteKeycodes": BITPropEdit,
...
class MaskedComboBoxDTC(ComboBoxDTC, MaskedDTCMixin, AutoFormatPropMixin):
    ...
    def hideDesignTime(self):
        return ComboBoxDTC.hideDesignTime(self) + MaskedDTCMixin.hideDesignTime(self)
...
class MaskedNumCtrlDTC(TextCtrlDTC, MaskedDTCMixin):
    def __init__(self, name, designer, parent, ctrlClass):
        ...
        self.editors.update(
            {
                "Min": PropertyEditors.BITPropEditor,
...
class TimeCtrlDTC(MaskedTextCtrlDTC):
    ...
    def properties(self):
        ...
        props["SpinButton"] = ("CompnRoute", self.GetSpinButton, self.BindSpinButton)
...
class ScrolledPanelDTC(ContainerCompanions.ScrolledWindowDTC, Constructors.WindowConstr):
    ...
    def writeImports(self):
        return "\n".join((ContainerCompanions.ScrolledWindowDTC.writeImports(self), "import wx.lib.scrolledpanel"))
...
class HyperLinkCtrlDTC(BasicCompanions.StaticTextDTC):
    ...
    def constructor(self):
        return {"Position": "pos", "Size": "size", "Label": "label", "Style": "style", "Name": "name", "URL": "URL"}
    ...
    def writeImports(self):
        return "\n".join((BasicCompanions.StaticTextDTC.writeImports(self), "import wx.lib.agw.hyperlink"))
...
class FileBrowseButtonDTC(PanelDTC):
    def __init__(self, name, designer, parent, ctrlClass):
        ...
        self.editors.update(
            {
                "LabelText": StrPropEdit,
                "ButtonText": StrPropEdit,
                "ToolTip": StrPropEdit,
                "DialogTitle": StrPropEdit,
                "StartDirectory": StrPropEdit,
                "InitialValue": StrPropEdit,
    ...
    def writeImports(self):
        return "\n".join((PanelDTC.writeImports(self), "import wx.lib.filebrowsebutton"))
...
class ThrobberDTC(PanelDTC):
    ...
    def writeImports(self):
        return "\n".join((PanelDTC.writeImports(self), "import wx.lib.throbber"))
...
try:
    ...
    Plugins.registerComponent(
        "Library", wx.lib.splitter.MultiSplitterWindow, "wx.lib.splitter.MultiSplitterWindow", MultiSplitterWindowDTC
    )
except ImportError:
    pass
...
try:
    ...
    Plugins.registerComponent(
        "Library", wx.lib.analogclock.AnalogClock, "wx.lib.analogclock.AnalogClock", AnalogClockDTC
    )
...
try:
    ...
    Plugins.registerComponents(
        ...
        (
            ...
            "wx.lib.filebrowsebutton.FileBrowseButtonWithHistory",
            FileBrowseButtonWithHistoryDTC,
        ),
