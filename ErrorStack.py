    ...
    def parse(self):
        if len(self.lines):
            ...
            for idx in range(len(self.lines) - 1):
                ...
                if mo:
                    self.stack.append(
                        StackEntry(mo.group("filename"), int(mo.group("lineno")), self.lines[idx + 1], self.error)
                    )
    ...
    def parse(self):
        ...
        if self.lines:
            ...
            try:
                ...
                self.error[:] = [warng]
                self.stack.append(
                    StackEntry(os.path.abspath(filename), lineNo, linecache.getline(filename, lineNo), self.error)
                )
...
,
 ...
 File "EditorModels.py", line 513
    ...
    tb2 = ['  File "Views\\SelectionTags.py", line 23\012', "    :\012", "    ^\012", "SyntaxError: invalid syntax\012"]
...
[File "Views\\SelectionTags.py", line 23
    ...
    single_line_excp = (
        "Exception exceptions.TypeError: 'call of non-function (type None)' in <method wxColourPtr.__del__ "
        "of wxColour instance at 03190B4C> ignored"
    )
    single_line_excp_answ = (
        """[['Exception exceptions.TypeError', "'call of non-function (type None)' in <method """
        + """wxColourPtr.__del__ of wxColour instance at 03190B4C> ignored"]\n[]]"""
    )
