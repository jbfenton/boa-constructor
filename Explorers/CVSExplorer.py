...
import Utils
from Models import EditorHelper, EditorModels
from Preferences import IS
from Utils import _

from . import ExplorerNodes, scrm
...
def isCVS(filename):
    ...
    return (
        file.lower() == "cvs"
        and os.path.exists(os.path.join(filename, "Entries"))
        and os.path.exists(os.path.join(filename, "Repository"))
        and os.path.exists(os.path.join(filename, "Root"))
    )
...
class CVSController(ExplorerNodes.Controller):
    ...
    def cvsCmdPrompt(self, wholeCommand, inDir, help=""):
        ...
        if wx.Platform == "__WXMSW__":
            ...
            try:
                te.SetSelection(wholeCommand.index("["), wholeCommand.index("]") + 1)
            except ValueError:
    ...
    def getCvsHelp(self, cmd, option="-H"):
        CVSPD = ProcessProgressDlg.ProcessProgressDlg(self.list, "cvs %s %s" % (option, cmd), "", modally=False)
        try:
    ...
    def doCvsCmd(self, cmd, cvsDir, stdinput="", cvsOutput="output window"):
        ...
        try:
            ...
            if cvsOutput == "output window":
                ...
                if err.strip():
                    dlg = wx.MessageDialog(self.list, err, _("Server response or Error"), wx.OK | wx.ICON_EXCLAMATION)
                    try:
                        dlg.ShowModal()
                    finally:
                        dlg.Destroy()
    ...
    def doCvsCmdOnSelection(self, cmd, cmdOpts, preCmdFunc=None, postCmdFunc=None, cvsOutput="output window"):
        if self.list.node:
            ...
            cmdStr = self.cvsCmdPrompt(self.cvsCmd(cmd, cmdOpts, names), cvsDir, self.getCvsHelp(cmd))
            if cmdStr:
                if preCmdFunc:
                    preCmdFunc(names)
    ...
    def doCvsCmdInDir(self, cmd, cmdOpts, cvsDir, items, cvsOpts=""):
        cmdStr = self.cvsCmdPrompt(self.cvsCmd(cmd, cmdOpts, items, cvsOpts), cvsDir, self.getCvsHelp(cmd))
        if cmdStr:
    ...
    def OnLoginCVS(self, event, cvsroot=""):
        ...
        cvsroot = self.cvsCmdPrompt(cvsroot, cvsDir, help=_("Change the CVSROOT if necessary:"))

        dlg = wx.TextEntryDialog(
            self.list,
            _("Enter cvs password for %s") % cvsroot,
            "CVS login",
            "",
    ...
    def OnEditEnv(self, event):
        ...
        dlg = wx.TextEntryDialog(
            self.list,
            _("Edit CVS shell environment variable: %s\nA blank entry will remove the variable.") % envKey,
            _("CVS shell environment variables"),
            envVal,
        )
        try:
            if dlg.ShowModal() == wx.ID_OK:
...
class CVSFolderNode(ExplorerNodes.ExplorerNode):
    ...
    def __init__(self, entriesLine, resourcepath, dirpos, parent):
        if entriesLine:
            name, self.revision, self.timestamp, self.options, self.tagdate = entriesLine[2:].split("/")
        else:
    ...
    def createParentNode(self):
        ...
        return FileSysNode(os.path.basename(parent), parent, self.clipboard, EditorModels.FolderModel.imgIdx, self)
...
class CVSFileNode(ExplorerNodes.ExplorerNode):
    ...
    def __init__(self, entriesLine, resourcepath, parent):
        if entriesLine:
            name, self.revision, self.timestamp, self.options, self.tagdate = entriesLine.strip()[1:].split("/")
        else:
            name = self.revision = self.timestamp = self.options = self.tagdate = ""
...
class FSCVSFolderNode(ExplorerNodes.ExplorerNode):
    ...
    def __init__(self, name, resourcepath, clipboard, imgIdx, parent, bookmarks=None):
        ExplorerNodes.ExplorerNode.__init__(
            self, name, resourcepath, clipboard, EditorModels.CVSFolderModel.imgIdx, parent
        )
        self.vetoSort = True
        self.dirpos = 0
        self.upImgIdx = 7
    ...
    def createParentNode(self):
        if self.parent:
            ...
            return FSCVSFolderNode(
                os.path.basename(parent), parent, self.clipboard, EditorModels.CVSFolderModel.imgIdx, self
            )
    ...
    def openList(self):
        ...
        for entry in fileEntries:
            ...
            if os.path.isdir(entry.resourcepath) and os.path.exists(testCVSDir) and isCVS(testCVSDir):
                node = CVSFolderNode("D/%s////" % entry.name, self.resourcepath, self.dirpos, self)
            else:
...
FileExplorer.FileSysNode.subExplorerReg["folder"].append((FSCVSFolderNode, isCVS, EditorHelper.imgCVSFolder))

ExplorerNodes.register(FSCVSFolderNode, clipboard=None, confdef=("", ""), controller=CVSController)
