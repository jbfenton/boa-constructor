...
class ExplorerClipboard:
    ...
    def clipPaste(self, node):
        if self.globClip.currentClipboard:
            ...
            clipMeth(node, self.globClip.currentClipboard.clipNodes, self.globClip.currentClipboard.clipMode)
...
class Controller:
    ...
    def createNode(self, category, name, resourcepath, uri):
        return self.Node(name, resourcepath, None, -1, None, None, properties={})
...
class ClipboardControllerMix:
    ...
    def __init__(self):
        ...
        self.clipMenuDef = [
            (wxID_CLIPRELOAD, _("Reload"), self.OnReloadItems, "-"),
            (-1, "-", None, "-"),
        ...
        if self.list.node:
            ...
            for node in nodes:
                if not node.isFolderish():
                    self.list.openNodeInEditor(node, self.editor, self.editor.explorerStore.recentFiles)
        ...
        if self.list.node:
            ...
            for node in nodes:
                if node.bookmarks:
                    ...
                    self.editor.statusBar.setHint(_("Bookmarked %s") % node.resourcepath, "Info")
            else:
                node = self.list.node
...
class ExplorerNode:
    ...
    def __init__(self, name, resourcepath, clipboard, imgIdx, parent=None, properties=None):
        ...
        self.category = ""
        self.stdAttrs = {"size": 0, "creation-date": 0.0, "modify-date": 0.0, "access-date": 0.0, "read-only": 0}
    ...
    def createChildNode(self, value):
        pass
...
class CategoryNode(ExplorerNode):
    ...
    def __init__(self, name, resourcepath, clipboard, config, parent, imgIdx=EditorHelper.imgFolder):
        ...
        self.config = config
        self.bold = True
        if self.protocol not in self.sharedEntries:
    ...
    def getConfigValue(self):
        try:
            return eval(self.config.get(self.resourcepath[cat_section], self.resourcepath[cat_option]), {})
    ...
    def deleteItems(self, names):
        for name in names:
            try:
                ...
                wx.LogWarning(_("Could not find %s in %s for deletion") % (name, list(self.entries.keys())))
        self.updateConfig()
    ...
    def updateConfig(self):
        assert isinstance(self.entries, type(self.__class__.entries)), _("Entries type %s invalid, expected %s") % (
            str(type(self.entries)),
            str(type(self.__class__.entries)),
        )
        self.config.set(self.resourcepath[cat_section], self.resourcepath[cat_option], pprint.pformat(self.entries))
...
(wxID_CATNEW, wxID_CATINSPECT, wxID_CATCOPY, wxID_CATDELETE, wxID_CATRENAME, wxID_CATRELOAD) = Utils.wxNewIds(6)
...
class CategoryController(Controller):
    ...
    def __init__(self, editor, list, inspector, controllers, menuDefs=[]):
        ...
        self.catMenuDef = [
            (wxID_CATNEW, _("New"), self.OnNewItem, self.newBmp),
...
class SubBookmarksCatNode(BookmarksCatNode):
    def __init__(self, parent, name, bookmarks):
        ...
        BookmarksCatNode.__init__(self, parent.clipboards, parent.config, parent, parent.catTransports, parent.tree)
        self.bold = False
...
class MRUCatNode(BookmarksCatNode):
    ...
    def __init__(self, clipboards, config, parent, catTransports, tree):
        BookmarksCatNode.__init__(
            self, clipboards, config, parent, catTransports, tree, _("Recent files"), ("explorer", "recentfiles")
        )
        self.vetoSort = True
...
from PropEdit.PropertyEditors import EvalConfPropEdit, PasswdStrConfPropEdit, StrConfPropEdit
...
class CategoryDictCompanion(CategoryCompanion):
    ...
    def setPropHook(self, name, value, oldProp=None):
        ...
        try:
            if self.name not in self.catNode.entries:
                raise Exception(
                    _("%s not found in the config, renaming config entries while Inspecting is not allowed.")
                    % self.name
                )
...
def register(Node, clipboard=None, confdef=("", ""), controller=None, category=None, root=False):
    ...
    explorerNodeReg[Node] = {"clipboard": clipboard, "confdef": confdef, "controller": controller, "category": category}
