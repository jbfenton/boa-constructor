...
class SSHController(ExplorerNodes.Controller, ExplorerNodes.ClipboardControllerMix):
    def __init__(self, editor, list, inspector, controllers):
        ...
        self.setupMenu(
            self.menu,
            self.list,
            [(wxID_SSHOPEN, "Open", self.OnOpenItems, "-"), (-1, "-", None, "")] + self.clipMenuDef,
        )
...
class SSHCatNode(ExplorerNodes.CategoryNode):
    itemProtocol = "ssh"
    defName = "SSH"
    defaultStruct = {"username": "", "cipher": "3des", "host": "", "root": "~"}

    def __init__(self, clipboard, config, parent, bookmarks):
        ExplorerNodes.CategoryNode.__init__(self, "SSH", ("explorer", "ssh"), clipboard, config, parent)
    ...
    def createChildNode(self, name, props):
        ...
        # if root and root[0] != '/':
        #    root = '/'+root
        itm = SSHItemNode(name, props, root, self.clipboard, True, EditorHelper.imgNetDrive)
        itm.category = name
...
class SSHItemNode(ExplorerNodes.ExplorerNode):
    ...
    def __init__(self, name, props, resourcepath, clipboard, isFolder, imgIdx):
        ExplorerNodes.ExplorerNode.__init__(self, name, resourcepath, clipboard, imgIdx, None, props)
    ...
    def execCmd(self, cmd):
        dlg = ProcessProgressDlg(None, self.sshCmd(cmd), _("SSH listing"), linesep="\012")
        try:
    ...
    def remotePath(self, filename):
        return "%(username)s@%(host)s:" % self.properties + self.resourcepath + (filename and "/" + filename or "")
    ...
    def copyFromFS(self, fsNode, fn=""):
        ...
        try:
            cmd = 'scp "%s" "%s"' % (os.path.basename(fsNode.resourcepath), self.remotePath(fn))
            self.execSCP(cmd)
        finally:
            os.chdir(cwd)
    ...
    def moveFileFrom(self, other):
        ...
        self.execCmd("mv '%s' '%s'" % (other.resourcepath, self.resourcepath + "/" + fn))

    def copyFileFrom(self, other):
        fn = os.path.basename(other.resourcepath)
        self.execCmd("cp '%s' '%s'" % (other.resourcepath, self.resourcepath + "/" + fn))
    ...
    def renameItem(self, name, newName):
        self.execCmd("mv '%s' '%s'" % (self.resourcepath + "/" + name, self.resourcepath + "/" + newName))
    ...
    def load(self, mode="rb"):
        ...
        try:
            ...
            if os.path.exists(fn):
                ...
                raise ExplorerNodes.TransportLoadError(_("File was not downloaded locally."), self.resourcepath)
    ...
    def save(self, filename, data, mode="wb", overwriteNewer=False):
        ...
        try:
            ...
            try:
                ...
                parentSSHNode = self.createChildNode(parentName, 1, self.properties, parentDir)
    ...
    def getNodeFromPath(self, respath):
        ...
        if isFolder:
            respath = respath[:-1]
        return self.createChildNode(os.path.basename(respath), isFolder, self.properties, respath)
...
ExplorerNodes.register(
    SSHItemNode, clipboard=SSHExpClipboard, confdef=("explorer", "ssh"), controller=SSHController, category=SSHCatNode
)
