...
class FileSysCatNode(ExplorerNodes.CategoryNode):
    ...
    def __init__(self, clipboard, config, parent, bookmarks):
        ExplorerNodes.CategoryNode.__init__(self, _("Filesystem"), ("explorer", "file"), clipboard, config, parent)
...
class FileSysController(ExplorerNodes.Controller, ExplorerNodes.ClipboardControllerMix):
    ...
    def __init__(self, editor, list, inspector, controllers):
        ...
        if "zip" in controllers:
            self.newMenuDef.append((wxID_FSNEWZIP, _("Empty zip archive"), self.OnEmptyZipArchive, "-"))
        if "tar.gz" in controllers:
            self.newMenuDef.append((wxID_FSNEWZIP, _("Empty tar.gz archive"), self.OnEmptyTarGzipArchive, "-"))
    ...
    def addFindResults(self, pattern, mapResults):
        ...
        if not isinstance(self.list.node, ResultsFolderNode):
            self.list.node = ResultsFolderNode(_("Results"), nd.resourcepath, nd.clipboard, -1, nd, nd.bookmarks)

        mapFindInFileCount = {}
        for oFindRes in list(mapResults.keys()):
            if len(mapResults[oFindRes]):
                mapFindInFileCount[oFindRes] = len(mapResults[oFindRes])
        self.list.node.results = map(None, list(mapFindInFileCount.values()), list(mapFindInFileCount.keys()))
    ...
    def OnSetAsSysCwd(self, event):
        ...
        if node:
            ...
            path = self.list.node.resourcepath
    ...
    def OnEmptyZipArchive(self, event):
        ...
        zipfile.ZipFile(os.path.join(self.list.node.resourcepath, newName), "w", zipfile.ZIP_DEFLATED).close()
...
class FileSysAttrPropEdit(PropertyEditors.PropertyEditor):
    def inspectorEdit(self):
        self.editorCtrl = InspectorEditorControls.BeveledLabelIEC(self, self.getValue())
...
class FileSysCompanion(ExplorerNodes.ExplorerCompanion):
    ...
    def getPropertyItems(self):
        ...
        for date in ("creation-date", "modify-date", "access-date"):
            value = attrs[date]
            if value:
                res.append((date, time.strftime(self.timeFmt, time.localtime(value))))
...
class FileSysExpClipboard(ExplorerNodes.ExplorerClipboard):
    ...
    def clipPaste_FTPExpClipboard(self, node, nodes, mode):
        ...
        for file in nodes:
            file.ftpConn.download(file.resourcepath, os.path.join(node.resourcepath, file.name))
...
class FileSysNode(ExplorerNodes.ExplorerNode):
    ...
    def __init__(self, name, resourcepath, clipboard, imgIdx, parent=None, bookmarks=None, properties={}):
        ExplorerNodes.ExplorerNode.__init__(self, name, resourcepath, clipboard, imgIdx, parent, properties or {})
        self.bookmarks = bookmarks
    ...
    def isFolderish(self):
        return self.isDir() or list(
            filter(
                lambda se, rp=self.resourcepath, ap=self.allowedProtocols: (
                    (ap == ["*"] or se[0].protocol in ap) and se[1](rp)
                ),
    ...
    def createParentNode(self):
        ...
        if parent[-2:] == "..":
            parent = parent[:-2]
        return FileSysNode(
            os.path.basename(parent), parent, self.clipboard, EditorHelper.imgFolder, self, self.bookmarks
        )
    ...
    def createChildNode(self, file, filename=""):
        ...
        if (".*" in exts or ext in exts) and os.path.isfile(filename):
            for Other, otherIdFunc, imgIdx in self.subExplorerReg["file"]:
                if "*" in self.allowedProtocols or Other.protocol in self.allowedProtocols:
                    if otherIdFunc(filename):
                        return "fol", Other(file, filename, self.clipboard, imgIdx, self, self.bookmarks)
            ...
            if ext in extSubTypes:
                for SubTypeModel in extSubTypes[ext]:
                    ...
                    return "", None
            return "mod", FileSysNode(file, filename, self.clipboard, Model.imgIdx, self, self.bookmarks, {})
        ...
        # Directories
            ...
            for other, otherIdFunc, imgIdx in self.subExplorerReg["folder"]:
                if (
                    self.filter == "BoaFiles"
                    and "*" in self.allowedProtocols
                    or "*" in self.allowedProtocols
                    or other.protocol in self.allowedProtocols
                ):
                    if otherIdFunc(filename):
                        return "fol", other(file, filename, self.clipboard, imgIdx, self, self.bookmarks)
            return "fol", FileSysNode(file, filename, self.clipboard, EditorHelper.imgFolder, self, self.bookmarks)
        else:
            return "", None
    ...
    def openList(self):
        try:
            ...
            if isinstance(self.resourcepath, str):
                ...
                files = os.listdir(Path(self.resourcepath))
    ...
    def getFilterExts(self):
        return {
            ...
            "ImageFiles": (EditorHelper.imageExtReg, EditorHelper.imageSubTypeExtReg),
            "AllFiles": ([".*"], {}),
        }.get(self.filter, (self.exts, {}))
        ...
        try:
            if not overwriteNewer and self.fileIsNewer():
                raise ExplorerNodes.TransportModifiedSaveError(
                    _("This file has been saved by someone else since it was loaded"), self.resourcepath
                )
    ...
    def updateStdAttrs(self):
        ...
        self.stdAttrs["read-only"] = exists and not os.access(self.resourcepath, os.W_OK)
        self.stdAttrs["modify-date"] = exists and os.stat(self.resourcepath)[stat.ST_MTIME] or 0.0
        self.stdAttrs["creation-date"] = exists and os.stat(self.resourcepath)[stat.ST_CTIME] or 0.0
        self.stdAttrs["access-date"] = exists and os.stat(self.resourcepath)[stat.ST_ATIME] or 0.0
        self.stdAttrs["size"] = exists and os.stat(self.resourcepath)[stat.ST_SIZE] or 0
    ...
    def fileIsNewer(self):
        return (os.path.exists(self.resourcepath) and os.stat(self.resourcepath)[stat.ST_MTIME] or 0.0) > self.stdAttrs[
            "modify-date"
        ]
...
class ResultsFolderNode(FileSysNode):
    ...
    def openList(self):
        ...
        for occrs, filename in self.results:
            tp, node = self.createChildNode("(%d) %s" % (occrs, filename), os.path.join(self.resourcepath, filename))
            if node:
                entries.append(node)
...
class CurWorkDirNode(FileSysNode):
    ...
    def __init__(self, clipboard, parent, bookmarks):
        ...
        FileSysNode.__init__(self, "os.cwd", self.cwd, clipboard, EditorHelper.imgPathFolder, parent)
        self.bookmarks = bookmarks
        self.bold = True
...
def findFileExplorerNode(category, respath, transports):
    ...
    raise ExplorerNodes.TransportError(_("FileSysCatNode not found in transports %s") % transports.entries)
...
ExplorerNodes.register(CurWorkDirNode, clipboard="file", controller="file", root=True)
