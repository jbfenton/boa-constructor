...
class ZipController(ExplorerNodes.Controller, ExplorerNodes.ClipboardControllerMix):
    def __init__(self, editor, list, inspector, controllers):
        ...
        self.setupMenu(
            self.menu,
            self.list,
            [(wxID_ZIPOPEN, _("Open"), self.OnOpenItems, "-"), (-1, "-", None, "")] + self.clipMenuDef,
        )
...
class ZipItemNode(ExplorerNodes.ExplorerNode):
    ...
    def __init__(self, name, resourcepath, clipboard, isFolder, imgIdx, parent, zipFileNode, ChildClass):
        ExplorerNodes.ExplorerNode.__init__(self, name, resourcepath, clipboard, imgIdx, parent)
    ...
    def createChildNode(self, name, resourcepath, isFolder):
        ...
        imgIdx = isFolder and EditorHelper.imgFolder or EditorHelper.imgTextModel
        if not isFolder:
        ...
        zin = self.ChildClass(
            name,
            resourcepath and resourcepath + "/" + name or name,
            self.clipboard,
            isFolder,
            imgIdx,
            self,
    ...
    def copyFromFS(self, fsNode, fn=""):
        ...
        if fsNode.isFolderish():
            ...
            # replace existing
            filesData = [(arcname, "", open(filename, "rb").read()) for arcname, filename in replace]
...
class ZipFileNode(ZipItemNode):
    ...
    def __init__(self, name, resourcepath, clipboard, imgIdx, parent, bookmarks=None):
        if clipboard:
            ...
            zipClip = None
        ZipItemNode.__init__(self, name, resourcepath, zipClip, True, imgIdx, parent, self, self.ChildClass)
        self.allFiles = None
        self.allFileNames = []
...
EditorHelper.imgZipFileModel = EditorHelper.addPluginImgs("Images/Modules/ZipFile.png")
...
FileExplorer.FileSysNode.subExplorerReg["file"].append((ZipFileNode, isZip, EditorHelper.imgZipFileModel))
ExplorerNodes.register(ZipItemNode, clipboard=ZipExpClipboard, controller=ZipController)
...
class TarGzipController(ZipController):
    pass
...
class TarGzipFileNode(TarGzipInfoMixin, ZipFileNode):
    ...
    def isDir(self, path=""):
        if path:
            try:
                idx = self.allFileNames.index(path)
            except ValueError:
                return path[-1] == "/"
...
EditorHelper.imgTarGzipFileModel = EditorHelper.addPluginImgs("Images/Modules/TarGzipFile.png")
...
FileExplorer.FileSysNode.subExplorerReg["file"].append((TarGzipFileNode, isTarGzip, EditorHelper.imgZipFileModel))
ExplorerNodes.register(TarGzipItemNode, clipboard=ZipExpClipboard, controller=TarGzipController)
