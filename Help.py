...
[
    wxID_PYDOCHELPPAGE,
    wxID_PYDOCHELPPAGEBOXRESULTS,
    wxID_PYDOCHELPPAGEBTNSEARCH,
    wxID_PYDOCHELPPAGEBTNSTOP,
    wxID_PYDOCHELPPAGECHKRUNSERVER,
    wxID_PYDOCHELPPAGEPNLSTATUS,
    wxID_PYDOCHELPPAGESTXSTATUS,
    wxID_PYDOCHELPPAGETXTSEARCH,
...
class PyDocHelpPage(wx.Panel):
    ...
    def _init_ctrls(self, prnt):
        ...
        self.txtSearch.SetConstraints(LayoutAnchors(self.txtSearch, True, True, True, False))
        ...
        self.boxResults.SetConstraints(LayoutAnchors(self.boxResults, True, True, True, True))
        self.boxResults.Bind(wx.EVT_LISTBOX, self.OnBoxresultsListboxDclick, id=wxID_PYDOCHELPPAGEBOXRESULTS)

        self.btnSearch = wx.Button(
        ...
        self.btnSearch.SetConstraints(LayoutAnchors(self.btnSearch, False, True, True, False))
        ...
        self.btnStop.SetConstraints(LayoutAnchors(self.btnStop, False, True, True, False))
        self.btnStop.Enable(False)
        self.btnStop.Bind(wx.EVT_BUTTON, self.OnBtnstopButton, id=wxID_PYDOCHELPPAGEBTNSTOP)

        self.chkRunServer = wx.CheckBox(
        ...
        self.pnlStatus = wx.Panel(
            id=wxID_PYDOCHELPPAGEPNLSTATUS,
            name="pnlStatus",
            parent=self,
            pos=wx.Point(80, 72),
            size=wx.Size(168, 16),
        ...
        self.stxStatus = wx.StaticText(
            id=wxID_PYDOCHELPPAGESTXSTATUS,
        ...
        self.stxStatus.SetConstraints(LayoutAnchors(self.stxStatus, True, True, True, False))
    ...
    def startPydocServer(self):
        ...
        if not testPydocServerAddress("localhost", 7464):
            ...
            threading.Thread(target=pydoc.serve, args=(7464, self.OnServerReady, self.OnServerQuit)).start()
    ...
    def OnServerReady(self, server):
        if self and self.helpFrame:
            ...
            wx.CallAfter(self.hyperlinkLabel, "http://localhost:%d/" % server.server_port)
    ...
    def doSearch(self):
        ...
        threading.Thread(target=self.scanner.run, args=(self.OnUpdateResults, key, self.OnFinishedResults)).start()
    ...
    def OnUpdateResults(self, path, modname, desc):
        if self:
            ...
            wx.CallAfter(self.boxResults.Append, modname + " - " + (desc or "(no description)"))
...
class wxHelpFrameEx:
    def __init__(self, helpctrlr):
        ...
        self.contentsAddBookmark, self.contentsDelBookmark, self.contentsChooseBookmark, self.contentsTree = (
    ...
    def IndexFind(self, text):
        ...
        wx.PostEvent(self.indexFindBtn, wx.CommandEvent(wx.wxEVT_COMMAND_BUTTON_CLICKED, self.indexFindBtn.GetId()))
...
def initHelp(calledAtStartup=False):
    ...
    if use_standard_controller:
        _hc = wx.html.HtmlHelpController(
            wxHF_ICONS_BOOK_CHAPTER | wxHF_DEFAULT_STYLE | (Preferences.flatTools and wxHF_FLAT_TOOLBAR or 0)
        )
    else:
        _hc = wxHtmlHelpControllerEx(
            wxHF_ICONS_BOOK | wxHF_DEFAULT_STYLE | (Preferences.flatTools and wxHF_FLAT_TOOLBAR or 0)
        )
        cf = wx.FileConfig(
            localFilename=os.path.normpath(jn(Preferences.rcPath, "helpfrm.cfg")), style=wx.CONFIG_USE_LOCAL_FILE
        )
        _hc.UseConfig(cf)
    ...
    for book in books:
        ...
        if os.path.exists(bookPath):
            _hc.AddBook(
                bookPath, not os.path.exists(jn(cacheDir, os.path.basename(book) + ".cached")) or not calledAtStartup
            )
