    ...
    def updateStaticTextPos(self):
        ...
        if self.StaticText:
            if self._captionAlignment == wx.TOP:
                self.StaticText.SetPosition((pos.x + offset.x, offset.y + pos.y - self.StaticText.GetSize().y - 5))
            elif self._captionAlignment == wx.LEFT:
                self.StaticText.SetPosition((offset.x + pos.x - self.StaticText.GetSize().x - 5, offset.y + pos.y))
