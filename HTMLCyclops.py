...
class CycleFinderHTML(Cyclops.CycleFinder):
        ...
        for i in range(n):
            ...
            if i < n - 1:
                if type(obj).__name__ == "instance method":
                    ...
                    self.report.append(
                        '%s<a href="attrlink://%s.%s">this%s</a>-><br>' % (indent, str(attrib_decl), attrib, attrib)
                    )
                else:
                    index = slice[i + 1][1]
                    ...
                    if attrib[0] == "." and hasattr(obj, "__class__"):
                        ...
                        self.report.append(indent + "this" + attrib + "-><br>")
    ...
    def show_obj(self, obj):
        ...
        self.report.append(
            '<font size="-1" color="#700060">%s</font> rc: %d <b>%s</b>' % (hex(objid), rc, type(obj).__name__)
        )
    ...
    def stats_list(self):
        ...
        return [
            ("Objects in root set", len(self.roots)),
    ...
    def iterate_til_steady_state(self, show_objs=0, summary=1):
        ...
        try:
            while True:
                ...
                x = None
                desc = (
                    '<br><b><font color="%s">%d</font> dead; <font color="%s">%d</font> non-cycle & alive</b><br>'
