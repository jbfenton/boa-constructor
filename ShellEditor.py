...
[
    wxID_SHELL_HISTORYUP,
    wxID_SHELL_HISTORYDOWN,
    wxID_SHELL_ENTER,
    wxID_SHELL_HOME,
    wxID_SHELL_CODECOMP,
    wxID_SHELL_CALLTIPS,
...
class ShellEditor(
    ...
    def OnKeyDown(self, event):
        ...
        if kk == wx.WXK_RETURN and not (shiftDown or event.HasModifiers()):
            ...
            if (
                self.lines.current == self.lines.count - 1
                and self.lines.pos - self.PositionFromLine(self.lines.current) < 5
            ):
                return
...
class PyCrustShellEditor(wx.SplitterWindow):
    def __init__(self, parent, wId):
        ...
        from wx.py.crust import Filling, Shell
        ...
        self.fillingWin = Filling(
            self, -1, style=wx.SP_3DSASH, rootObject=self.shellWin.interp.locals, rootIsNamespace=True
        )
...
shellReg = {"Shell": (ShellEditor, EditorHelper.imgShell), "PyCrust": (PyCrustShellEditor, EditorHelper.imgPyCrust)}
