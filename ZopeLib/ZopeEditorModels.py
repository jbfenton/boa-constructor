...
import Preferences
import Utils
from Models import EditorHelper, EditorModels, HTMLSupport
...
class ZopeEditorModel(EditorModels.BasePersistentModel):
    ...
    def save(self, overwriteNewer=False):
        ...
        try:
            ...
            raise ExplorerNodes.TransportSaveError(Utils.html2txt(err.faultString), self.transport.resourcepath)
...
class ZopeDocumentModel(ZopeEditorModel):
    ...
    def getPageName(self):
        if self.transport.name == "index_html":
            return "%s (%s)" % (self.transport.name, self.transport.resourcepath.split("/")[-2])
        else:
...
class ZopePythonSourceModel(ZopeDocumentModel):
    ...
    def getModule(self):
        if self._module is None:
            ...
            wx.BeginBusyCursor()
            try:
                self._module = moduleparse.Module(self.transport.whole_name, self.data.split("\012"))
            finally:
                wx.EndBusyCursor()
        return self._module
        ...
        for cat in transports.entries:
            if cat.itemProtocol == "zope":
                ...
                for itm in itms:
                    ...
                    if lp:
                        localZopes[itm.treename] = itm.properties
                if not localZopes:
...
(Hint: Set the localpath property of the Zope connection)""")
                    ...
                    dlg = wx.SingleChoiceDialog(
                        ...
                        "Choose Zope instance",
                        list(localZopes.keys()),
                    )
                    try:
                        if dlg.ShowModal() == wx.ID_OK:
                            ...
                            zexppath = (
                                props["localpath"] + "/import/" + os.path.basename(model.transport.resourcepath)
                            ).replace("<LocalFS::directory>", "<LocalFS::file>")

                            model.load()
                            model.saveAs(zexppath)

                            self.editor.setStatus(".zexp file copied. Ready to import in Zope.")
                    finally:
                        dlg.Destroy()
...
class ZopeController(Controllers.PersistentController):
    ...
    def OnViewInBrowser(self, event):
        model = self.getModel()
        model.editor.explorer.controllers["zope"].openSelItemInBrowser(zopeItem=model.transport)
