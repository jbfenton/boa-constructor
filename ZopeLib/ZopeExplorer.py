...
from .ZopeCompanions import ZopeCompanion, ZopeConnection
...
class ZopeEClip(ExplorerNodes.ExplorerClipboard):
    def __init__(self, globClip, props):
        ...
        self.zc.connect(props["host"], props["httpport"], props["username"], props["passwd"])
    ...
    def clipPaste_ZopeEClip(self, node, nodes, mode):
        mime, res = self.zc.call(node.resourcepath, "manage_pasteObjects", cb_copy_data=self.clipRef)
    ...
    def clipPaste_FileSysExpClipboard(self, node, nodes, mode):
        for file in nodes:
            if file.isDir():
                ...
                self.clipPaste_FileSysExpClipboard(folderNode, file.openList(), mode)
            else:
...
class ZopeCatNode(ExplorerNodes.CategoryNode):
    ...
    def __init__(self, globClip, config, parent, bookmarks):
        ExplorerNodes.CategoryNode.__init__(self, "Zope", ("explorer", "zope"), None, config, None)
    ...
    def createChildNode(self, name, props):
        ...
        zin = ZopeItemNode(
            "", props["path"], clipboard, EditorHelper.imgZopeConnection, self, None, None, props, "Folder"
        )
        zin.category = name
        zin.treename = name
...
class ZopeItemNode(ExplorerNodes.ExplorerNode):
    ...
    def __init__(self, name, resourcepath, clipboard, imgIdx, parent, xmlrpcsvr, root, properties, metatype):
        ExplorerNodes.ExplorerNode.__init__(self, name, resourcepath, clipboard, imgIdx, None, properties)
    ...
    def createChildNode(self, metatype, id, respath=None):
        ...
        if itm.imgIdx == -1:
            itm.imgIdx = ZopeEditorModels.ZOAIcons.get(metatype, ZopeEditorModels.ZOAIcons["unknown"])
    ...
    def checkentry(self, name, metatype, path):
        ...
        return ZopeNodeClass(*(name, path, self.clipboard, -1, self, self.server, self.root, self.properties, metatype))
    ...
    def getResource(self, url=""):
        if not url:
            url = self.buildUrl()
        return getServer(url, self.properties["username"], self.properties["passwd"])
        ...
        try:
            ...
            try:
                ...
                if (
                    ...
                    conninfo = ("http://%s" % self.buildUrl(), self.properties["username"], self.properties["passwd"])
                    ZoaClient.installFromFS(
                        conninfo,
                        os.path.join(
                            Preferences.pyPath,
                            "ZopeLib",
    ...
    def deleteItems(self, names):
        mime, res = self.clipboard.zc.call(self.resourcepath, "manage_delObjects", ids=names)

    def renameItem(self, name, newName):
        mime, res = self.clipboard.zc.call(self.resourcepath, "manage_renameObject", id=name, new_id=newName)

    def exportObj(self):
        mime, res = self.clipboard.zc.call(
            os.path.dirname(self.resourcepath), "manage_exportObject", download=1, id=self.name
        )
        return res

    def uploadObj(self, content):
        mime, res = self.clipboard.zc.call(self.resourcepath, "manage_upload", file=content)
        return res
    ...
    def newItem(self, name, Compn, getNewValidName=True):
        ...
        cmp.connect(props["host"], props["httpport"], props["username"], props["passwd"])
        cmp.create()
    ...
    def uploadFromFS(self, filenode):
        ...
        r = Resource(
            ("http://%(host)s:%(httpport)s/" + self.resourcepath + "/" + filenode.name) % props,
            props["username"],
            props["passwd"],
        )
        r.put(filenode.load())
    ...
    def getNodeFromPath(self, respath, metatype):
        return self.createChildNode(metatype, os.path.basename(respath), os.path.dirname(respath))

    def findItems(self, obj_ids=(), obj_metatypes=None, obj_searchterm=None, search_sub=1):
...
(
    ...
    wxID_ZOPEIMPORT,
    wxID_ZOPEINSPECT,
    wxID_ZOPEOPENINEDITOR,
    wxID_ZOPEUPLOAD,
    wxID_ZOPESECURITY,
    wxID_ZOPEUNDO,
    wxID_ZOPEVIEWBROWSER,
    wxID_ZCCSTART,
    wxID_ZCCRESTART,
    wxID_ZCCSHUTDOWN,
    wxID_ZCCTEST,
...
class ZopeCatController(ExplorerNodes.CategoryController):
    ...
    def __init__(self, editor, list, inspector, controllers, menuDefs=[]):
        ...
        ExplorerNodes.CategoryController.__init__(
            self, editor, list, inspector, controllers, menuDefs=menuDefs + zccMenuDef
        )
                    ...
                    try:
                        ...
                        if err[0] == 10061:
                            ...
                            if (
                                ...
                                and wx.MessageBox(
                                    "Keep checking for Zope to become available",
                                    "Retry?",
    ...
    def getControlPanel(self, props):
        return getServer("%(host)s:%(httpport)d/Control_Panel" % props, props["username"], props["passwd"])

    def callControlPanelMethod(self, props, meth):
        zc = ZopeConnection()
        zc.connect(props["host"], props["httpport"], props["username"], props["passwd"])
    ...
    def OnTest(self, event):
        for node in self.getNodesForSelection(self.list.getMultiSelection()):
            wx.LogMessage("%s : %s" % (node.treename, self.checkAvailability(node.properties, 0)))

    def OnOpenZopeLog(self, event):
        for node in self.getNodesForSelection(self.list.getMultiSelection()):
            props = node.properties
            if props["localpath"]:
                self.editor.openOrGotoModule(os.path.join(props["localpath"], "var", "Z2.log"))
            else:
    ...
    def OnConfigureZ2py(self, event):
        ...
        props = node.properties
        if props["localpath"]:
            cfgZ2SrcNode = ZopeZ2pySourceBasedPrefColNode("Z2.py", ("*",), props["localpath"] + "/z2.py", -1, node)
...
class ZopeController(ExplorerNodes.Controller, ExplorerNodes.ClipboardControllerMix):
    ...
    def __init__(self, editor, list, inspector, controllers):
        ...
        self.zopeMenuDef = (
            [(wxID_ZOPEINSPECT, "Inspect", self.OnInspectItem, self.inspectBmp), (-1, "-", None, "")]
            + self.clipMenuDef
            + [
                (-1, "-", None, ""),
        ...
        if self.list.node:
            ...
            for idx in idxs:
                ...
                if item:
                    ...
                    dlg = wxFileDialog(
                        self.list, "Save as...", currPath, item.name + ".zexp", "", wx.SAVE | wx.OVERWRITE_PROMPT
                    )
                    try:
        ...
        if self.list.node:
            ...
            for idx in idxs:
                ...
                if item:
                    ...
                    dlg = wxFileDialog(self.list, "Upload " + item.name, currPath, item.name, "", wx.FD_OPEN)
                    try:
                        if dlg.ShowModal() == wx.ID_OK:
                            try:
        ...
        if self.list.node:
            ...
            if zopeItem:
                ...
                if viewName not in model.views:
                    resultView = self.editor.addNewView(viewName, ZopeViews.ZopeSecurityView)
                else:
    ...
    def OnUndoZopeItem(self, event):
        if self.list.node:
            ...
            if zopeItem and ZopeViews.ZopeUndoView in zopeItem.additionalViews:
                ...
                if viewName not in model.views:
                    resultView = self.editor.addNewView(viewName, ZopeViews.ZopeUndoView)
                else:
                    resultView = model.views[viewName]
                resultView.refresh()
                resultView.focus()
        ...
        if node:
            ...
            try:
                if dlg.ShowModal() == wx.ID_OK:
                    ...
                    wx.BeginBusyCursor()
                    try:
                        results = node.findItems(obj_ids, meta_type, search_text, search_sub)
                    finally:
                        wx.EndBusyCursor()
...
def getServer(url, user, password):
    return xmlrpclib.Server("http://" + url, BasicAuthTransport.BasicAuthTransport(user, password))
...
class ControlNode(DirNode):
    def checkentry(self, id, entry, path):
        if entry == "Product Management":
            childnode = PMNode(id, path, self.clipboard, -1, self, self.server, self.root, self.properties, entry)
        else:
...
class PMNode(ControlNode):
    def checkentry(self, id, entry, path):
        if entry == "Product":
            childnode = ProductNode(id, path, self.clipboard, -1, self, self.server, self.root, self.properties, entry)
        else:
...
class ProductNode(DirNode):
    def checkentry(self, id, entry, path):
        if entry == "Z Class":
            childnode = ZClassNode(id, path, self.clipboard, -1, self, self.server, self.root, self.properties, entry)
        else:
            childnode = DirNode.checkentry(self, id, entry, path)
        return childnode
...
class PythonNode(ZopeNode):
    ...
    additionalViews = (ZopeViews.ZopeSecurityView, Views.EditorViews.ToDoView, ZopeViews.ZopeUndoView)
    ...
    def save(self, filename, data, mode="wb"):
        self.getResource().manage_edit(self.name, self.getParams(data), self.getBody(data))


class PythonScriptNode(PythonNode):
    additionalViews = (ZopeViews.ZopeSecurityView, ZopeViews.ZopeUndoView, Views.EditorViews.ToDoView)
    ...
    def save(self, filename, data, mode="wb"):
        self.getResource().ZPythonScriptHTML_editAction("fake", self.name, self.getParams(data), self.getBody(data))


class ExtPythonNode(PythonNode):
    Model = ZopeEditorModels.ZopeExternalMethodModel
    defaultViews = (Views.PySourceView.PythonSourceView, Views.EditorViews.ExploreView)
    additionalViews = (
        Views.EditorViews.HierarchyView,
        Views.EditorViews.ModuleDocView,
        ZopeViews.ZopeSecurityView,
        Views.EditorViews.ToDoView,
...
class DTMLMethodNode(ZopeNode):
    ...
    defaultViews = (ZopeViews.ZopeHTMLSourceView,)
    additionalViews = (ZopeViews.ZopeUndoView, ZopeViews.ZopeSecurityView, ZopeViews.ZopeHTMLView)
...
def uriSplitZope(filename, filepath):
    ...
    segs = filepath.split("/")
    if len(segs) < 2:
        raise ExplorerNodes.TransportCategoryError("Category not found", filepath)
...
def uriSplitZopeDebug(filename, filepath):
    ...
    segs = filepath.split("/")
    if len(segs) < 3:
        raise ExplorerNodes.TransportCategoryError("Zope debug path invalid", filepath)
    ...
    for cat in ExplorerNodes.all_transports.entries:
        if cat.itemProtocol == "zope":
            ...
            for itm in itms:
                ...
                # @@@ Gogo.
                if socket.gethostbyname(props["host"]) == socket.gethostbyname(host) and props["httpport"] == port:
    ...
    raise ExplorerNodes.TransportCategoryError("Could not map Zope debug path to defined Zope Category item", filepath)
...
def findZopeExplorerNode(catandmeta, respath, transports):
    ...
    raise ExplorerNodes.TransportError("Zope transport could not be found: %s || %s" % (category, respath))
...
ExplorerNodes.register(
    ZopeItemNode, clipboard="global", confdef=("explorer", "zope"), controller=ZopeController, category=ZopeCatNode
)
