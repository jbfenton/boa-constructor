...
class ZopeCompanion(ExplorerCompanion, ZopeConnection):
    ...
    def getPropertyMap(self):
        ...
        if len(self.propItems) == 2:
            ...
            return [{"id": self.propItems[0][0], "type": tpe1}, {"id": self.propItems[1][0], "type": tpe2}]
        else:
    ...
    def setPropHook(self, name, value, oldProp):
        mime, res = self.callkw(self.objPath, "manage_changeProperties", {name: value})
        return True
...
class CustomZopePropsMixIn:
    def getPropertyMap(self):
        ...
        for propName in self.propOrder:
            propMap.append({"id": propName, "type": self.propTypeMap[propName][0]})
        return propMap
...
class PythonScriptZC(CustomZopePropsMixIn, ZopeCompanion):
    def create(self):
        ...
        mime, res = self.call(
            self.objPath, prodPath + "manage_addPythonScript", id=self.name, title="", params="", body="pass"
        )
...
class ExternalMethodZC(CustomZopePropsMixIn, ZopeCompanion):
    ...
    def SetProp(self, name, value):
        ...
        mime, res = self.call(
            self.objPath, "manage_edit", title=props["title"], module=props["module"], function=props["function"]
        )
...
class ZCatalogZC(ZopeCompanion):
    def create(self):
        mime, res = self.call(self.objPath, "manage_addProduct/ZCatalog/manage_addZCatalog", id=self.name, title="")
...
class SQLMethodZC(CustomZopePropsMixIn, ZopeCompanion):
    ...
    propOrder = ("title", "connection_id", "max_rows", "max_cache", "cache_time", "class_file", "class_name")
...
class GadflyDAZC(DBAdapterZC):
    def create(self):
        dlg = wx.TextEntryDialog(None, "Enter the Data Source", "Z Gadfly DB Adapter", "")
        try:
            if dlg.ShowModal() == wx.ID_OK:
                connId = dlg.GetValue()
                mime, res = self.call(
                    self.objPath, "manage_addZGadflyConnection", id=self.name, title="", connection=connId, check=1
                )
        finally:
            dlg.Destroy()
...
class VersionZC(ZopeCompanion):
    def create(self):
        mime, res = self.call(self.objPath, "manage_addProduct/OFSP/manage_addVersion", id=self.name, title="")
...
class UserZC(CustomZopePropsMixIn, ZopeCompanion):
    ...
    def create(self):
        dlg = wx.TextEntryDialog(
            None,
            "Enter the username:\n(The password will be set to this username and\ncan be updated in the Inspector)",
            "New User",
            "",
        )
        try:
            if dlg.ShowModal() == wx.ID_OK:
                ...
                mime, res = self.call(
                    self.objPath, "manage_users", submit="Add", name=username, password=username, confirm=username
                )
        finally:
            dlg.Destroy()
...
PaletteStore.paletteLists["Zope"].extend(
    [
        "Folder",
        "DTML Document",
        "DTML Method",
