...
(
    ...
    imgFramePanelModel,
    imgWizardModel,
    imgPyWizardPageModel,
    imgWizardPageSimpleModel,
...
class BaseFrameModel(ClassModel):
    ...
    def renameMain(self, oldName, newName):
        ...
        if "create" in self.getModule().functions:
            self.getModule().replaceFunctionBody("create", ["    return %s(parent)" % newName, ""])
    ...
    def new(self, params):
        ...
        for param in list(params.keys()):
            paramLst.append(Preferences.cgKeywordArgFormat % {"keyword": param, "value": params[param]})
        ...
        self.data = (
            sourceconst.defSig + self.defImport + self.defCreateClass + self.defWindowIds + self.defClass
        ) % srcValsDict
    ...
    def readDesignerMethod(self, meth, codeBody):
        ...
        # Normal method
            ...
            allInitialisers, unmatched = methodparse.parseMixedBody(
                [
                    methodparse.ConstructorParse,
                    methodparse.EventParse,
                    methodparse.CollectionInitParse,
                    methodparse.PropertyParse,
                ],
                codeBody,
            )
        ...
        if unmatched:
            wx.LogWarning(_("The following lines were not used by the Designer and will be lost:\n"))
    ...
    def readSpecialAttrs(self, mod, cls):
        ...
        if extAttrInitName:
            if extAttrInitName not in mod.from_imports_names:
                raise Exception(
                    "%s.__init__ called, but not imported in the form: "
            ...
            extModFilename = os.path.join(os.path.dirname(self.filename), extModName + ".py")
            from Explorers.Explorer import openEx

            try:
                data = openEx(extModFilename).load()
            except Exception:
                raise Exception("Problem loading %s: File expected at: %s" % (extModName, extModFilename))
            ...
            readAttrsFromSrc(attrs, extClass.attributes, extModule.source, extMeth.start, extMeth.end)
    ...
    def readComponents(self):
        ...
        if self.main in module.classes:
            ...
            self.resources = self.readResources(module, main, specialAttrs=self.specialAttrs)
            ...
            for oc in self.identifyCollectionMethods():
                ...
                for prop in self.objectCollections[oc].properties[:]:
                    if prop.asText() in ("self.%s()" % sourceconst.init_utils, "self.%s()" % sourceconst.init_sizers):
            ...
            if len(lst) > 1 and Preferences.cgWrapLines:
                ...
                for seg in lst[1:]:
                    ...
                    if len(newLine) >= Preferences.cgLineWrapWidth:
                        ...
                        line = newLine
                lines.append(line)
                lines.append((sourceconst.defWindowIdsCont % {"idIdent": colMeth, "idCount": len(lst)}).strip())
            else:
                lines.append(
                    (
                        sourceconst.defWindowIds % {"idNames": ", ".join(lst), "idIdent": colMeth, "idCount": len(lst)}
                    ).strip()
                )
            lines.append("")
...
class AppModel(BaseAppModel):
    ...
    def renameMain(self, oldName, newName):
        ...
        self.getModule().replaceFunctionBody(
            "main", ["    application = %s(0)" % newName, "    application.MainLoop()", ""]
        )

    def new(self, mainModule):
        self.data = (sourceconst.defEnvPython + sourceconst.defSig + sourceconst.defImport + sourceconst.defApp) % {
            "modelIdent": self.modelIdentifier,
...
class FramePanelModel(BaseFrameModel):
    ...
    def __init__(self, data, name, main, editor, saved, app=None):
        ...
        self.defClass = sourceconst.defClass.replace("parent", "parent, id, pos, size, style, name", 1)
