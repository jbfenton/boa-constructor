...
[
    wxID_RESOURCESELECTDLG,
    wxID_RESOURCESELECTDLGBTNCANCEL,
    wxID_RESOURCESELECTDLGBTNFILEDLG,
    wxID_RESOURCESELECTDLGBTNOK,
...
class ResourceSelectDlg(wx.Dialog):
    def _init_coll_boxSizerButtons_Items(self, parent):
        ...
        parent.AddWindow(self.btnOK, 0, border=15, flag=wx.BOTTOM | wx.TOP | wx.LEFT | wx.ALIGN_RIGHT)
        parent.AddWindow(self.btnCancel, 0, border=15, flag=wx.BOTTOM | wx.TOP | wx.LEFT | wx.ALIGN_RIGHT)
        parent.AddWindow(self.btnFileDlg, 0, border=15, flag=wx.BOTTOM | wx.TOP | wx.LEFT | wx.ALIGN_RIGHT)
    ...
    def _init_ctrls(self, prnt):
        ...
        wx.Dialog.__init__(
            self,
            id=wxID_RESOURCESELECTDLG,
            name="ResourceSelectDlg",
            parent=prnt,
            pos=wx.Point(384, 293),
        ...
        self.btnOK = wx.Button(
            id=wx.ID_OK, label=_("OK"), name="btnOK", parent=self, pos=wx.Point(15, 15), size=wx.Size(75, 23), style=0
        )
    ...
    def __init__(self, parent, editor, resourceFilename, imageName="", onlyIcons=False):
        ...
        self.resources = PyResourceImagesSelectionView(
            self, model, listStyle=wx.LC_SMALL_ICON | wx.LC_ALIGN_TOP, imgLstStyle=wx.IMAGE_LIST_SMALL
        )
        self.resources.onlyIcons = onlyIcons

        self.boxSizerMain.Prepend(self.resources, 1, wx.LEFT | wx.RIGHT | wx.TOP | wx.GROW, 15)
        self.resources.refreshCtrl()

        if imageName:
            sel = -1
            for idx, r in zip(list(range(len(self.resources.imageSrcInfo))), self.resources.imageSrcInfo):
                if r[0] == imageName:
                    sel = idx
                    break
...
import Utils
from Models import Controllers, EditorHelper, PythonControllers, PythonEditorModels
...
class PyResourceImagesView(EditorViews.ListCtrlView):
    ...
    def __init__(self, parent, model, listStyle=wx.LC_ICON | wx.LC_ALIGN_TOP, imgLstStyle=wx.IMAGE_LIST_NORMAL):
        ...
        try:
            ...
            for f in m.function_order:
                if f.startswith("get") and f.endswith("Data"):
                    ...
                    self.imageSrcInfo.append((name, (m.functions[f].start, bmpFunctionStart), compressed, iconFunction))
        finally:
            wx.ArtProvider.Pop()
    ...
    def OnExportImage(self, event):
        if self.selected != -1:
            ...
            dlg = wx.FileDialog(
                self, "Save image", ".", name + ".png", ";".join(["*%s" % e for e in extTypeMap]), wx.SAVE
            )
            try:
                if dlg.ShowModal() == wx.ID_OK:
                    path = dlg.GetPath()
...
class PyResourceBitmapController(PythonControllers.ModuleController):
    ...
    DefaultViews = PythonControllers.ModuleController.DefaultViews + [PyResourceImagesView]
...
def ConvertImgToPy(imgPath, editor):
    ...
    if ok:
        ...
        dlg = wx.TextEntryDialog(editor, _("Options:\n\n%s\n\nEdit options string:") % docs, "img2py", cmdLine)
        try:
            if dlg.ShowModal() != wx.ID_OK:
                return
...
if ".py" not in EditorHelper.imageSubTypeExtReg:
