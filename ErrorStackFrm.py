...
[
    wxID_ERRORSTACKMFSTATUSBAR,
    wxID_ERRORSTACKMFERRORSTACKTC,
    wxID_ERRORSTACKMF,
    wxID_ERRORSTACKMFNOTEBOOK,
    wxID_ERRORSTACKMFOUTPUTTC,
    wxID_ERRORSTACKMFERRORTC,
...
class ErrorStackMF(wx.Frame, Utils.FrameRestorerMixin):
    ...
    def _init_ctrls(self, prnt):
        ...
        self.statusBar = wx.StatusBar(id=wxID_ERRORSTACKMFSTATUSBAR, parent=self, name="statusBar", style=0)
        self.SetStatusBar(self.statusBar)

        self.outputTC = wx.TextCtrl(
            size=wx.Size(326, 384),
            value="",
            pos=wx.Point(0, 0),
    ...
    def __init__(self, parent, editor):
        ...
        if Preferences.eoErrOutNotebookStyle == "text":
            self.tracebackImgIdx = self.outputImgIdx = self.errorsImgIdx = self.diffImgIdx = self.inputImgIdx = (
                self.processesImgIdx
            ) = -1
        ...
        self.errorStackTC.Bind(
            wx.EVT_TREE_ITEM_ACTIVATED, self.OnErrorstacktcTreeItemActivated, id=wxID_ERRORSTACKMFERRORSTACKTC
        )
        self.errorStackTC.Bind(
            wx.EVT_TREE_SEL_CHANGED, self.OnErrorstacktcTreeSelChanged, id=wxID_ERRORSTACKMFERRORSTACKTC
        )
    ...
    def setDefaultDimensions(self):
        self.SetDimensions(
            0,
            Preferences.paletteHeight + Preferences.windowManagerTop + Preferences.windowManagerBottom,
    ...
    def addTracebackNode(self, err, parsedTracebacks):
        ...
        if err.error and err.stack:
            ...
            for si in err.stack:
                siTI = tree.AppendItem(errTI, "%d: %s: %s" % (si.lineNo, os.path.basename(si.file), si.line.strip()))
    ...
    def updateCtrls(self, errorList, outputList=None, rootName="Error", runningDir="", errRaw=None, addToHistory=True):
    ...
    def appendToTextCtrl(self, tc, txt, TEXTCTRL_MAXLEN=30000, TEXTCTRL_GOODLEN=20000):
    ...
    def displayDiff(self, diffResult):
        if not self.diffPage:
            self.diffPage = wx.stc.StyledTextCtrl(self.notebook, -1, style=wx.SUNKEN_BORDER | wx.CLIP_CHILDREN)
            ...
            self.notebook.AddPage(
                text=_("Diffs"), select=not not diffResult, page=self.diffPage, imageId=self.diffImgIdx
            )
    ...
    def displayInput(self, display=True):
        if not self.inputPage:
            self.inputPage = wx.TextCtrl(
                self.notebook, -1, value="", style=wx.TE_MULTILINE | wx.TE_RICH | wx.SUNKEN_BORDER | wx.CLIP_CHILDREN
            )
            self.inputPage.Bind(wx.EVT_LEFT_DCLICK, self.OnInputDoubleClick)
            self.notebook.InsertPage(self.displayPageIdx, self.inputPage, _("Input"), True, self.inputImgIdx)
    ...
    def displayProcesses(self):
        if not self.processesPage:
            self.processesPage = wx.ListView(self.notebook, -1, style=wx.LC_LIST | wx.LC_ALIGN_TOP)
            self.notebook.AddPage(self.processesPage, self.processesText, False, self.processesImgIdx)
        else:
        ...
        if self.processesPage:
            ...
            if cnt:
                ...
                if (
                    wx.MessageBox(
                        _(
                            "There are still running processes that were "
    ...
    def stepBackInHistory(self):
        if len(self.history) > 1:
            ...
            self.updateCtrls(*(self.history[self.historyIdx] + (False,)))
