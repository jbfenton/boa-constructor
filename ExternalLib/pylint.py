...
for name in (
    ...
    + [].append.__members__
    + warn.__members__
    + warn.func_code.__members__
    + tb.__members__
    + tb.tb_frame.__members__
...
reserved = {}
for name in str.split(
    "abs add and bases builtin builtins call class cmp "
...
def count(token_type, token, start, end, line, seen=special.has_key, imported=modules.has_key):
        ...
        try:
            if last == "from":
                ...
                try:
                    ...
                    pass
            if token_type != tokenize.NAME or seen(token):
                return
            if opt_i and not modfrom and sawimport and not imported(token):
                try:
                    ...
                    pass
            if parent and member.has_key(token):
                pass
            # RB:
            ...
            # --
                ...
                special[token] = 1
            elif parent and ignore_subobjs and not opt_i:
                pass
            elif len(token) >= 3 and token[:2] == "wx" and token[2] in string.uppercase:
                pass
            elif len(token) >= 5 and token[:4] == "EVT_":
                pass
            # --
            elif imported(parent) and modules[parent].has_key(token):
                pass
                ...
                if not reserved.has_key(token):
                    warn(filename, srow, line, 'dubious reserved name "%s"' % token, scol, ecol)
            elif last != "from":
                position[token] = (srow, line, scol, ecol, bracket_depth)
...
def pylint(afile, filename):
    ...
    try:
        tokenize.tokenize(afile.readline, count)
    ...
    for linenum, (start, end, name, line, brk_dpth) in unique:
        warn(
            ...
            linenum,
            line,
            ('"%s" used only once (%d)', '"%s" defined but unused (%d)')[defined.has_key(name)] % (name, brk_dpth),
            start,
            end,
        )
