...
class Upgrade:
    def __init__(self):
        ...
        self.specialNames = {
            "GenButton": "wx.lib.buttons.GenButton",
        ...
        evt_P2 = Literal("EVT_") + uident + LPAREN + qualIdent + COMMA + qualIdent + RPAREN
        evt_P2.setParseAction(self.evt_P2Action)

        # 3 Parameter evt macros.
        evt_P3 = Literal("EVT_") + uident + LPAREN + qualIdent + COMMA + qualIdent + COMMA + qualIdent + RPAREN
        evt_P3.setParseAction(self.evt_P3Action)

        # 3 Parameter evt macros.
        #   specialEventCode = True required
        #   event codes containing "control.GetId()" in code
        evt_P3a = Literal("EVT_") + uident + LPAREN + qualIdent + COMMA + qualIdent2 + COMMA + qualIdent + RPAREN
        ...
        append = Suppress(".Append") + LPAREN + karg + COMMA + karg + COMMA + karg + COMMA + karg + RPAREN
        append.setParseAction(self.appendAction)

        # wxFont
        wxfont = Suppress("wxFont") + LPAREN + intOnly + COMMA + ident + COMMA + ident + COMMA + ident
        wxfont.setParseAction(self.wxfontAction)

        # SetStatusText keyword args
        setStatusText = Suppress(".SetStatusText") + LPAREN + ident + EQ + intOnly
        setStatusText.setParseAction(self.setStatusTextAction)

        # AddSpacer keyword args
        addSpacer = Suppress(".AddSpacer") + LPAREN + intOnly + COMMA + intOnly
        ...
        COL = Suppress(":")
        repId1 = Suppress("map(lambda") + qualIdent2 + COL + qualIdent2 + COMMA + RANGE + intOnly + RPAREN + RPAREN
        repId1.setParseAction(self.repId1Action)

        # import
        imp = Literal("from wxPython.") + delimitedList(ident, ".", combine=True) + Literal("import") + restOfLine
        ...
        custClass = (
            Literal("_custom_classes")
            + EQ
            + LBRACE
            + dictOf(dictKey, COLON + dictVal + Optional(COMMA)).setResultsName("dictData")
            + RBRACE
        )
