...
def CreateLanguagesResourceLists(filter=LC_AVAILABLE, only=()):
    ...
    for li in _wxLangIds:
        ...
        try:
            if (
                (filter == LC_ONLY and wxLI in only)
                or (filter == LC_AVAILABLE and wx.Locale.IsAvailable(wxLI))
                or (filter == LC_ALL)
            ):
    ...
    langCodes = [(langIdNameMap[wxLangId], wxLangId) for wxLangId in wxLangIds if wxLangId in langIdNameMap]
