...
def transportInstalled(transport):
    return transport in eval(Utils.createAndReadConfig("Explorer").get("explorer", "installedtransports"), {})
...
def buildPluginExecList():
    ...
    for globPath, (ordered, disabled) in pluginPathGlobs:
        ...
        for pluginName in ordered:
            pluginFilename = os.path.join(os.path.dirname(globPath), pluginName) + ".plug-in.py"
            try:
        ...
        for pluginName in disabled:
            disabledPlugins.append(os.path.join(os.path.dirname(globPath), pluginName) + ".plug-in.py")

        for pluginFilename in globList:
            pluginExecList.append(
                (pluginFilename, pluginFilename in orderedPlugins, pluginFilename not in disabledPlugins)
            )
...
def updateRcFile(rcFile, propName, propSrcValue):
    ...
    prefsRcNode = UsedModuleSrcBsdPrefColNode("", ("*",), prefsRcFile, -1, None, Preferences, True)
...
def registerFileType(Controller, Model=None, newName="", addToNew=True, aliasExts=()):
...
def registerPalettePage(paletteName, paletteTitle):
    ...
    if paletteName not in PaletteStore.paletteLists:
        ...
        PaletteStore.palette.append([paletteTitle, "", PaletteStore.paletteLists[paletteName]])
...
def registerLanguageSTCStyle(name, lang, STCClass, stylesFile, insertPos=None):
    ...
    if insertPos is not None:
        ExplorerNodes.langStyleInfoReg.insert(insertPos, (name, lang, STCClass, stylesFile))
    else:
        ExplorerNodes.langStyleInfoReg.append((name, lang, STCClass, stylesFile))
...
def registerPreference(pluginName, prefName, defPrefValSrc, docs=[], info=""):
    ...
    if prefName not in m.globals:
        ...
        raise PluginError(_("%s not in Preferences, but is defined in globals of prefs_plugins_rc.py") % prefName)
