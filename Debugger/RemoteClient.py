...
class TransportWithAuthentication(xmlrpclib.Transport):
    ...
    def __init__(self, user, pw):
        self._auth = "basic %s" % base64.encodestring("%s:%s" % (user, pw)).strip()
...
class RemoteClient(MultiThreadedDebugClient):
    ...
    def invoke(self, m_name, m_args):
        if self.server is None:
            ...
            url = "http://%s:%d/RemoteDebug" % (self.host, int(self.port))
    ...
    def kill(self):
        if self.server is not None:
            ...
            self.taskHandler.addTask(self.server.set_disconnect, ())
        self.server = None
